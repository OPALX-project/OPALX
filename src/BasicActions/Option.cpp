	 ...
	 "The \"OPTION\" statement defines OPAL execution options.")
{
  ...
  itsAttr[PSDUMPEACHTURN] = Attributes::makeBool
    ("PSDUMPEACHTURN", "If true, dump phase space after each turn in local frame (one cannot restart from here), its default value is false");
  ...
  Action(name, parent)
{
  ...
  Attributes::setReal(itsAttr[PSDUMPEACHTURN], psDumpEachTurn);
