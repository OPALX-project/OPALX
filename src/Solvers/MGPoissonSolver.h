...
public:
  ...
  /// constructor
  MGPoissonSolver(Mesh_t *mesh, FieldLayout_t *fl, std::vector<BoundaryGeometry*> geometries, std::string itsolver, std::string interpl, double tol, int maxiters, std::string precmode);
...
private:
  ...
  /// \param hr gridspacings in each direction
  /// \return Epetra_CrsMatrix holding the stencil
  inline void Stencil3DGeometry(Vector_t hr, Epetra_CrsMatrix*& Matrix, Epetra_Vector& RHS);
  //inline Epetra_CrsMatrix* Stencil3DGeometry(Vector_t hr, Epetra_Vector& RHS);
