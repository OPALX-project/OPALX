...
void TaperDomain::Compute(Vector_t hr)
{
    ...
    switch(interpolationMethod) {
        ...
        case QUADRATIC:
            ...
            for(x=0; x<nr[0]; x++) {
                pos = (x-(nr[0]-1)/2)*hr[0]; 
                yd = abs(sqrt(rb2 - pos*pos));
                IntersectYDir.insert(pair<int, double>(x, yd));
                IntersectYDir.insert(pair<int, double>(x, -yd));
                yd = abs(sqrt(rs2 - pos*pos));
                Intersectydir.insert(pair<int, double>(x, yd));
                Intersectydir.insert(pair<int, double>(x, -yd));
            }
            ...
            for(y=0; y<nr[1]; y++) {
                pos = (y-(nr[1]-1)/2)*hr[1]; 
                xd = abs(sqrt(rb2 - pos*pos));
                IntersectXDir.insert(pair<int, double>(y, xd));
                IntersectXDir.insert(pair<int, double>(y, -xd));
                xd = abs(sqrt(rs2 - pos*pos));
                Intersectxdir.insert(pair<int, double>(y, xd));
                Intersectxdir.insert(pair<int, double>(y, -xd));
            }
    }
...
void TaperDomain::getBoundaryStencil(int x, int y, int z, double& W, double& E, double& S, double& N, double& F, double& B, double& C, double& scaleFactor) {
    ...
    switch(interpolationMethod) {
        case CONSTANT:  ConstantInterpolation(x,y,z,W,E,S,N,F,B,C,scaleFactor); break;
        case LINEAR:    LinearInterpolation(x,y,z,W,E,S,N,F,B,C,scaleFactor);   break;
        case QUADRATIC: QuadraticInterpolation(x,y,z,W,E,S,N,F,B,C,scaleFactor); break;
    }
