...
FFTPoissonSolver::FFTPoissonSolver(Mesh_t *mesh, FieldLayout_t *fl, string greensFunction):
  layout3_m(0),    
  mesh3_m(0),
  layout2_m(0),    
  mesh2_m(0),
...
{ 
  ...
  GreensFunctionTimer_m = IpplTimings::getTimer("GreensFunctionTotal");
  
  IntGreensFunctionTimer1_m = IpplTimings::getTimer("IntGreensFunction1");
  IntGreensFunctionTimer2_m = IpplTimings::getTimer("IntGreensFunction2");
  IntGreensFunctionTimer3_m = IpplTimings::getTimer("IntGreensFunction3");
  IntGreensFunctionTimer4_m = IpplTimings::getTimer("IntGreensFunction4");

  ShIntGreensFunctionTimer1_m = IpplTimings::getTimer("ShIntGreensFunction1");
  ShIntGreensFunctionTimer2_m = IpplTimings::getTimer("ShIntGreensFunction2");
  ShIntGreensFunctionTimer3_m = IpplTimings::getTimer("ShIntGreensFunction3");
  ShIntGreensFunctionTimer4_m = IpplTimings::getTimer("ShIntGreensFunction4");

  GreensFunctionTimer1_m = IpplTimings::getTimer("GreensFunction1");
  GreensFunctionTimer2_m = IpplTimings::getTimer("GreensFunction2");
  GreensFunctionTimer3_m = IpplTimings::getTimer("GreensFunction3");
  GreensFunctionTimer4_m = IpplTimings::getTimer("GreensFunction4");
}


FFTPoissonSolver::FFTPoissonSolver(PartBunch &beam, string greensFunction):
  layout3_m(0),    
  mesh3_m(0),
  layout2_m(0),    
  mesh2_m(0),
...
/** If the beam has a longitudinal size >> transverse size the 
  ...
  *
  * \f[ \overline{G}(x_i - x_{i'}, y_j - y_{j'}, z_k - z_{k'}) = \int_{x_{i'} - h_x/2}^{x_{i'} + h_x/2} dx' \int_{y_{j'} - h_y/2}^{y_{j'} + h_y/2} dy' \int_{z_{k'} - h_z/2}^{z_{k'} + h_z/2} dz' G(x_i - x_{i'}, y_j - y_{j'}, z_k - z_{k'}).
  * \f]
  */
void FFTPoissonSolver::integratedGreensFunction() 
{
  ...
  /**
    ...
    */
  for(int k=idx[2].first(); k < min(nr_m[2]+2,idx[2].last()+3); k++) {
    for(int j=idx[1].first(); j < min(nr_m[1]+2,idx[1].last()+3); j++) {
      for(int i=idx[0].first(); i < min(nr_m[0]+2,idx[0].last()+3); i++) {
...
void FFTPoissonSolver::shiftedIntGreensFunction(double zshift) {
  ...
  */
  ...
  rho2_m[I][J][K] -= tmpgreen[I][J][K];
  tmpgreen = 0.0;
  //later reoplace ggrn2 with tmpgreen
