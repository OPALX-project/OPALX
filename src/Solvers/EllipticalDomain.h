...
///PointList maps from an (x) resp. (y) to a list of double values (=intersections with boundary) 
//typedef std::map< int, std::vector<double> > PointList;
//int encodes point
//double intersection value
typedef std::multimap<int, double> PointList;
...
public:
  ...
  /// constructor
  EllipticalDomain(double semimajor, double semiminor, Vector_t nr, Vector_t hr);
  ...
  inline bool isInside(int x, int y, int z) {
    double xx = (x-floor(nr[0]/2))*hr[0];
    double yy = (y-floor(nr[1]/2))*hr[1];
    return (xx*xx/(SemiMajor*SemiMajor) + yy*yy/(SemiMinor*SemiMinor) <= 1);
  }
  ...
  /// access method for discretization
  //std::vector<double> getBoundaryMeshSpacing(int idx);
...
private:
  ...
  /// maps irregular mesh spaces (near the border) to an idx.
  /// This is used as a lookup table when scaling the RHS
  std::map<int, vector<double> > BoundaryGridPoints;
  /// insert a irregular grid point into the list
  //void insertBoundaryMeshSpacing(int idx, double value, int direction);
