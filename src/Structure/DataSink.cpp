...
void DataSink::writePhaseSpace(PartBunch &beam, Vector_t FDext[], double sposHead, double sposRef, double sposTail) {
    ...
    if(writeDummy = (nLoc == 0)) {
        WARNMSG("Zero particles on a node add a dummy particle" << endl);
        nLoc = 1;
    }
    ...
    if(writeDummy) {
        ...
        // FixMe: need H5PartWriteDataInt32 or H5PartWriteDataShort
...
void DataSink::stashPhaseSpaceEnvelope(EnvelopeBunch &beam, Vector_t FDext[], double sposHead, double sposRef, double sposTail) {
    ...
    stash_actPos.push_back(beam.get_sPos() + sshift_m);
    ...
    stash_sposHead.push_back(sposHead + sshift_m);
    stash_sposRef.push_back(sposRef + sshift_m);
    stash_sposTail.push_back(sposTail + sshift_m);
    stash_Bhead.push_back(FDext[0]);
    stash_Ehead.push_back(FDext[1] * 1e-6);
    stash_Bref.push_back(FDext[2]);
    stash_Eref.push_back(FDext[3] * 1e-6);
    stash_Btail.push_back(FDext[4]);
    stash_Etail.push_back(FDext[5] * 1e-6);
...
void DataSink::writePartlossZASCII(PartBunch &beam, BoundaryGeometry &bg, string fn) {
    ...
    size_t temp = (vtkWrites_m > 0) ? (vtkWrites_m - 1) : 0;
    string ffn = fn + convert2Int(temp) + string(".dat");
    ...
    for(int i = 0; i < Geo_nr(2) ; i++) {
        ...
        for(int j = 0; j < bg.numbfaces_global_m; j++) {
            if(((Geo_mincoords[2] + Geo_hr(2)*i) < bg.Tribarycent_m[j](2)) && (bg.Tribarycent_m[j](2) < (Geo_hr(2)*i + Geo_hr(2) + Geo_mincoords[2]))) {
    ...
    fid << "# Delta_Z" << std::setw(15) << "Zcoordinates" << std::setw(15) << "numPrParticles" << std::setw(15) << "numSeParticles" << std::setw(15) << "t" << endl ;

    double t = beam.getT();
    for(int i = 0; i < Geo_nr(2) ; i++) {
        size_t primaryPLoss = bg.TriPrPartlossZ_m[i];
        size_t secondaryPLoss = bg.TriSePartlossZ_m[i];
        reduce(primaryPLoss, primaryPLoss, OpAddAssign());
        reduce(secondaryPLoss, secondaryPLoss, OpAddAssign());
        fid << Geo_hr(2) << std::setw(15) << Geo_mincoords[2] + Geo_hr(2)*i << std::setw(15) << primaryPLoss << std::setw(15) << secondaryPLoss << std::setw(15) << t << endl;
    }
    delete ofp;
    vtkWrites_m++;
}
...
void DataSink::writeGeomToVtk(BoundaryGeometry &bg, string fn) {
    if(Ippl::myNode() == 0) {
        ...
        for(int i = 0; i < bg.numbfaces_global_m ; i ++) {
            if((bg.TriPrPartloss_m[i] != 0) || (bg.TriSePartloss_m[i] != 0))
