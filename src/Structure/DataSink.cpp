...
DataSink::DataSink(int restartStep)
{
    ...
    // Test if .stat file exists. If it does, append new data to existing file.
    fn = OPAL.getInputFn();
    ...
    ofstream statFile(statFileName_m.c_str(), ios::app);
    ofstream lBalFile(lBalFileName_m.c_str(), ios::app);
    ...
    if (OPAL.hasRestartFile()){
        ...
        if (fn.find(string("_Part"),0) == string::npos){
            ...
            if (numint > 99999){
                ERRORMSG("Maximal \"h5\" file number is limited to 99999 !" << endl);
                exit(0);
            }
            ...
            *gmsg<<"New phase space data will dump into newfile: "<<fn<<endl;
        }
...
void DataSink::writePhaseSpace(PartBunch &beam, Vector_t FDext[], double sposHead, double sposRef, double sposTail) {
    ...
    */
    ...
    /**
       mpart and qi could be a file attribute 
    */
