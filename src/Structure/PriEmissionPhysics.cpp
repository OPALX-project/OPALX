 ...
 */
void PriEmissionPhysics::Fieldemission(
	PartBunch *itsBunch, const double &fa, const double &Enormal, const double &parameterFNB,
    ...
    std::vector<size_t> count(node_num, 0);
    ...
    double Jtmp = fa * Enormal * Enormal * exp(- parameterFNB *  workFunction * sqrt(workFunction) * (parameterFNVYZe -  parameterFNVYSe * pow(parameterFNY * sqrt(fieldEnhancement * (-Enormal)) /  workFunction, 2)) /  fieldEnhancement / (-Enormal));
    ...
    if(Jchild < Jtmp) {
        ...
        for(size_t k = 0; k < N; k++) {
            ...
            if(pc == Ippl::myNode()) {
                ...
                itsBunch->create(1);
                itsBunch->R[count[pc]] = (vertex[0] + r1 * (vertex[1] -  vertex[0]) +
					  r2 * (1 - r1) * (vertex[2] -  vertex[0])) +  TriNormal * tmpd ;
		...
		// During our simulation, we find some emitted particles will
		// not move if they just on the surface. So here we also add
		//margins =  TriNormal * tmpd from the emission surface to make
		// sure that the emitted particles will "feel" the RF field.
                itsBunch->R[count[pc]] =  vertex[0] + r1 * (vertex[1] -  vertex[0]) +
			r2 * (1 - r1) * (vertex[2] -  vertex[0]) +  TriNormal * tmpd;
                itsBunch->P[count[pc]] = Vector_t(0.0);
                itsBunch->Bin[count[pc]] = 0;
