	     ...
	     "The \"FIELDSOLVER\" statement defines data for a the field solver ")
{
  ...
  itsAttr[FSTYPE] = Attributes::makeString("FSTYPE", "Name of the attached field solver: FFT, MG, xxxx ");
...
void FieldSolver::initSolver(PartBunch &b) 
{
  ...
  if(Attributes::getString(itsAttr[FSTYPE])=="FFT") {
    ...
    solver_m = new MGPoissonSolver(mesh_m,FL_m,geometries,Attributes::getString(itsAttr[ITSOLVER]),Attributes::getString(itsAttr[INTERPL]),Attributes::getReal(itsAttr[TOL]),(int)Attributes::getReal(itsAttr[MAXITERS]));
    itsBunch_m->set_meshEnlargement(Attributes::getReal(itsAttr[BBOXINCR])/100.0);
    fsType_m = "MG";
#else
...
Inform &FieldSolver::print(Inform &os) const
{
  ...
  if(Attributes::getString(itsAttr[FSTYPE]) == "FFT")
    ...
    os << "* GEOMETRY     " << Attributes::getString(itsAttr[GEOMETRY]) << '\n'     
       ...
       << "* MAXITERS     " << Attributes::getReal(itsAttr[MAXITERS]) << endl;
  if (Attributes::getBool(itsAttr[PARFFTX]))
