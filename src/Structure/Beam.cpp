...
void Beam::update()
{
  ...
  static const char *names[] = {
    "ELECTRON",   "PROTON",     "POSITRON",   "ANTIPROTON",  "CARBON", "HYDRONGEN_MINUS"
  };

  static const double masses[] = {
    m_e, m_p, m_e, m_p, m_c, m_hm
  };

  static const double charges[] = {
    -1.0,         1.0,          1.0,          -1.0,     12.0,   -1.0     
  };
...
Inform &Beam::print(Inform &os) const
{
  ...
  os << "* BEAM        " << getOpalName() << '\n'
     ...
     << "* CURRENT     " << Attributes::getReal(itsAttr[BCURRENT]) << '\n'
     << "* FREQENCY    " << Attributes::getReal(itsAttr[BFREQ]) << '\n'
     << "* CHARGE      " << Attributes::getReal(itsAttr[CHARGE]) << '\n'
     << "* MASS        " << Attributes::getReal(itsAttr[MASS]) << '\n'
