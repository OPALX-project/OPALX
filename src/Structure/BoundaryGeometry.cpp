...
string BoundaryGeometry::getFilename() const {
    return (string)Attributes::getString(itsAttr[FGEOM]);
}

string BoundaryGeometry::getTopology() const {
...
std::vector<string> BoundaryGeometry::getDistributionArray() {
...
double BoundaryGeometry::getXYZScale() {
        ...
        for(int i = 0; i < numbfaces_global_m; i++) {
            ...
            if(((Tribarycent_m[i](2) > z_lower - 0.00001) && (Tribarycent_m[i](2) < z_upper + 0.00001)) &&
               (((Tribarycent_m[i](1) > y_ptest - 0.00001) && (Tribarycent_m[i](1) < y_ptest + 0.00001))
                ...
                // Other positions can have obsorption, FNEmission and SecondaryEmission.
                temp = ((BGphysics::Absorption)
...
bool  BoundaryGeometry::isInside(Vector_t x) {
    ...
    /*=================================================debug=====================================================================*/
    std::ofstream of;
    of.open(string("data/testLine.vtk").c_str());
    assert(of.is_open());
    of.precision(6);
    of << "# vtk DataFile Version 2.0" << std::endl;                      // first line of a vtk file
    of << "generated using BoundaryGeometry::makeBoundaryIndexSet" << std::endl;   // header
    of << "ASCII" << std::endl << std::endl;                                   // file format
    of << "DATASET UNSTRUCTURED_GRID" << std::endl;                 // dataset structrue
    of << "POINTS " << 2 << " float" << std::endl;  // data num and type

    of << x0[0] << " " << x0[1]  << " " << x0[2] << std::endl;

    of << x1[0] << " " << x1[1]  << " " << x1[2] << std::endl;



    of << "CELLS " << 1 << " " << 3 << std::endl;

    of << "2 " << 0 << " " << 1 << std::endl;
    of << "CELL_TYPES " << 1 << std::endl;
    of << "3" << std::endl;
    of << std::endl;
    == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == * /
#endif
...
void  BoundaryGeometry::initialize() {
    ...
    *gmsg << "* Iniitializing Boundary Geometry ... ..." << endl;
    ...
    // not needed all additional write out go to the data dir
    //    string dir("data");
    ...
    *gmsg << "* Scale all points of the geometry by " << xyzscale_m << endl;
...
#if defined(USE_H5FED)
    rc = H5SetErrorHandler(H5AbortErrorhandler);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    rc = H5SetVerbosityLevel(1);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    h5_file_t *f = H5OpenFile(h5FileName_m.c_str(), H5_O_RDONLY, Ippl::getComm());
    rc = H5FedOpenTriangleMesh(f, 0);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    rc = H5FedSetLevel(f, 0);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);

    numbfaces_global_m = H5FedGetNumElementsTotal(f);
    ...
    while((local_id = H5FedTraverseEntities(f, iter)) >= 0) {
        ...
        rc = H5FedGetVertexIndicesOfEntity(f, local_id, local_vids);
        if(rc != H5_SUCCESS)
            ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
        allbfaces_m[4*i]   = 0;
    ...
    rc = H5FedEndTraverseEntities(f, iter);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);

    // loop over all vertices
    numpoints_global_m = H5FedGetNumVerticesTotal(f);
    double *point_coords = new double[3 * numpoints_global_m];
    for(i = 0; i < numpoints_global_m; i++) {
        h5_float64_t P[3];
        rc = H5FedGetVertexCoordsByIndex(f, i, P);
        if(rc != H5_SUCCESS)
            ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    ...
    rc = H5Pset_fapl_mpio(plist_id, Ippl::getComm(), MPI_INFO_NULL);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    ...
    hid_t dset_id = H5Dopen2(file_id, "/surface", H5P_DEFAULT);
    assert(dset_id >= 0);
    hid_t x = H5Dget_space(dset_id);
    rc = H5Sget_simple_extent_dims(x, dimsf, NULL);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    ...
    // read dataset collective.
    hid_t plist_id2 = H5Pcreate(H5P_DATASET_XFER);
    rc = H5Pset_dxpl_mpio(plist_id2, H5FD_MPIO_COLLECTIVE);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    ...
    /*
      heronion outputs an index,
      case 0 indicates no symmetry plane,
      1 for (x,y) sym,
      2 for (y,z),
      3 for (z,x),none 0 means the current triangle is not on a real surface of geometry but on a symmetry plane.
    */
    for(int i = 0; i < numbfaces_global_m; i ++) {
        if(allbfaces_m[4*i] > 0) {
            nof_sym_m++;
            numbfaces_global_m--;
            for(int j = i; j < numbfaces_global_m; j++) {
                allbfaces_m[4*j]   = allbfaces_m[4*(j+1)];
                allbfaces_m[4*j+1] = allbfaces_m[4*(j+1)+1];
                allbfaces_m[4*j+2] = allbfaces_m[4*(j+1)+2];
                allbfaces_m[4*j+3] = allbfaces_m[4*(j+1)+3];
            }
        }
    }
    rc = H5Dclose(dset_id);
    ...
    *gmsg << "*  Surface index read in done." << endl;
    ...
    ////////////////////////////////
    hsize_t dimsf_c[2];
    dset_id = H5Dopen2(file_id, "/coords", H5P_DEFAULT);
    assert(dset_id >= 0);

    // Create the dataspace for the dataset.
    hid_t cp_space = H5Dget_space(dset_id);
    rc = H5Sget_simple_extent_dims(cp_space, dimsf_c, NULL);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    ...
    rc = H5Pset_dxpl_mpio(plist_id3, H5FD_MPIO_COLLECTIVE);
    if(rc != H5_SUCCESS)
        ERRORMSG("H5 rc= " << rc << " in " << __FILE__ << " @ line " << __LINE__ << endl);
    status = H5Dread(dset_id, H5T_NATIVE_DOUBLE, H5S_ALL, filespace_c, plist_id3, point_coords);
    assert(status >= 0);
    ...
    *gmsg << "*  Triangle barycent built done." << endl;
    ...
    /*
      ...
      cases, we need to use smaller bounding box size to speed up simulation
      by setting a larger nr_m, for example use nr_m(0) /= 0.2 in the following.
...
void  BoundaryGeometry::createPriPart(
    ...
    if(Options::ppdebug) {
        ...
        int Parent = 0;
        if(Ippl::myNode() == 0) {
            /* limit the initial particle in the center of the lower parallel
               plate. There is a distance of 0.01*length in x direction as
               margin. */
            double x_low = mincoords_m(0) + 0.5 * len_m(0) - 0.49 * len_m(0);

            /* limit the initial particle in the center of the upper parallel
               plate. There is a distance of 0.01*length in x direction as
               margin. */
...
void BoundaryGeometry::makeBoundaryIndexSet() {
    ...
    for(int i = 0; i < numbfaces_global_m; i++) {
        ...
        for(int j = 0; j < 200; j++) {
            ...
            discreteTri.push_back(geo3Dcoords_m[allbfaces_m[4*i+3]] + 0.005 * (j + 1) * (0.5 * (geo3Dcoords_m[allbfaces_m[4*i+1]] + geo3Dcoords_m[allbfaces_m[4*i+2]]) - geo3Dcoords_m[allbfaces_m[4*i+3]]));
            ...
            discreteTri.push_back(geo3Dcoords_m[allbfaces_m[4*i+3]] + 0.005 * (j + 1) * (geo3Dcoords_m[allbfaces_m[4*i+1]] - geo3Dcoords_m[allbfaces_m[4*i+3]]));
        }
    ...
    of << "ASCII" << std::endl << std::endl;                                   // file format
    of << "DATASET UNSTRUCTURED_GRID" << std::endl;                 // dataset structrue
    of << "POINTS " << numpoints << " float" << std::endl;  // data num and type
...
int BoundaryGeometry::PartInside(
    ...
    if(isInGeometry(temp1)) {
        ...
        /* Build an array contains the ids of 27(3*3*3) cubic boxes. The id of
           the cubic box which contains the particle, lies in the center of the
           27 cubic boxes. Just for the situation that even the line segment
        ...
        for(int k = 0; k < 27; k++) {
            ...
            /* Lookup table (stl map) contains the id of cubic box as key
               and triangle ids inside the id th cubic box as value */

            if(It != CubicLookupTable_m.end()) {
                /* If cubic box among the 27 cubic boxes is not the boundary
                   bounding box, do not need to be tested at all. If cubic box
                   is the boundary bounding box then do the following tests. */
                for(myIt = (*It).second.begin(); myIt != (*It).second.end(); myIt++) {
                    /* For each particle inside the cubic box, test if this
                       particle have intersect with each triangle inside the
                       same cubic box */
                    if(v != 0 &&
                       dot(v, TriNormal_m[*myIt]) <= 0.0) {
                        /* If the particle have none zero momenta and momenta
                           has opposite direction with triangle normal, do the
                           following tests. */
                        IpplTimings::startTimer(TRayTrace_m);
                        // Ray triangle test
                        FindIntersection(temp1,         // IN: particle position in tstep n
                        ...
                        if(Isc != out_m) {
                            ...
                            if((rI >= -0.00001 && rI <= 1.00001) ||
                               (rI < 0 && dot(temp1 - Tribarycent_m[*myIt], TriNormal_m[*myIt]) <= 0.0)) {
                                flg++;
                                intecoords = Isc;
                                triId = (*myIt);
                                ...
                                if(TriPrPartloss_m[*myIt] > 0 ||
                                   TriSePartloss_m[*myIt] > 0 ||
                                   TriFEPartloss_m[*myIt] > 0) {
                                    std::cout << "* Loss Data" << *myIt << " : "
                                              << TriPrPartloss_m[*myIt] << " "
                                              << TriSePartloss_m[*myIt] << " "
                                              << TriFEPartloss_m[*myIt] << std::endl;
                                }
                                break;
                            }
                        }
        ...
        /* Build an array contains the ids of 27(3*3*3) cubic boxes. The
           id of the cubic box which contains the particle, lies in the
           center of the 27 cubic boxes. Just for the situation that even
           the line segment cross more than one cubic box. */
        int idc[27] = {
            id,                                 id + 1,                                 id - 1,
            id + nr_m(0),                       id - nr_m(0),                           id + nr_m(0) + 1,
            id + nr_m(0) - 1,                   id - nr_m(0) - 1,                       id - nr_m(0) + 1,
