...
public:
     ...
     * 
     */
    void retriveCavityInformation(string fn);
    ...
    void writePartlossZASCII(PartBunch &beam, BoundaryGeometry &bg, string fn);
     ...
     *
     */
    void writeGeomToVtk(BoundaryGeometry &bg, string fn);
    //void writeGeoContourToVtk(BoundaryGeometry &bg, string fn);
     ...
     *
     */
    void writeImpactStatistics(PartBunch &beam, long long int &step, size_t &impact, double &sey_num, bool nEmissionMode, string fn);

    void writeSurfaceInteraction(PartBunch &beam, long long int &step, BoundaryGeometry &bg, string fn);
...
private:
    ...
    string surfacLossFileName_m;

    /// %Pointer to H5 file for surface loss data.
    H5PartFile *H5fileS_m;
    ...
    struct file_size_name {
        off_t file_size_m;
        string file_name_m;
        file_size_name(const string &fn, const off_t &fs) {
            file_name_m = fn;
            file_size_m = fs;
        }
