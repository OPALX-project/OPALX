...
void Wake::initWakefunction(ElementBase &element)
{
  ...
  *gmsg << "* **********************************************************************************"<< endl;
  ...
  vector<string> filters_str = Attributes::getStringArray(itsAttr[FILTERS]);
  vector<OpalFilter*> filters;
  
  for (vector<string>::const_iterator fit = filters_str.begin(); fit != filters_str.end(); ++ fit) {
  ...
  if (Attributes::getString(itsAttr[TYPE]) == "1D-CSR") 
    {
      wf_m = new CSRWakeFunction(reference, itsElement_m, filters, (int)(Attributes::getReal(itsAttr[NBIN])));
    }
  else if (Attributes::getString(itsAttr[TYPE]) == "LONG-SHORT-RANGE") 
    {
      ...
      wf_m = new GreenWakeFunction(reference, filters, (int)(Attributes::getReal(itsAttr[NBIN])), (Attributes::getReal(itsAttr[Z0])), (Attributes::getReal(itsAttr[RADIUS])), 
