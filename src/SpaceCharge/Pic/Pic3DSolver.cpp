...
namespace opalx::spacecharge {
    ...
    Pic3DSolver::Pic3DSolver(
            ...
            std::shared_ptr<Workspace> workspace, BackendFacade& backendFacade, DataSink* dataSink)
        : primary_m(&requirePrimaryParticles(particleContainers)),
          workspace_m(std::move(workspace)),
          backendFacade_m(&backendFacade),
          dataSink_m(dataSink),
        ...
        if (&backendFacade_m->getWorkspace() != workspace_m.get()) {
            throw OpalException(
                    "Pic3DSolver::Pic3DSolver",
                    "The backend facade and 3D solver must share one PIC workspace.");
        }
    ...
    void Pic3DSolver::execute(SolveContext& context, SelfFieldDiagnostics& diagnostics) {
        ...
        IpplPoissonAdapter& backend = backendFacade_m->getBackendAdapter();
    ...
    void Pic3DSolver::solveInBeamFrame(
        ...
        const auto& backendCapabilities = backendFacade_m->getBackendCapabilities();
        ...
        m << level4 << "Entry: rank=" << ippl::Comm->rank()
          ...
          << ", stype=" << backendFacade_m->getStype() << endl;
    ...
    void Pic3DSolver::executeSolvePass(
        ...
        const auto& capabilities = backendFacade_m->getBackendCapabilities();
        if (direct) {
            ...
            normalization.couplingConstant      = backendFacade_m->getCouplingConstant();
        ...
        try {
            ...
            backendFacade_m->runSolver(backendRequest, pass.suppressFieldDump, diagnostics);
