 ...
 * @brief Defines the native particle and field bindings used by one self-field call.
 */
...
namespace opalx::spacecharge {
    ...
    template <typename T, unsigned Dim>
    [[nodiscard]] ParticleFieldBinding<T, Dim> bindParticleFields(
    ...
    /** @brief Non-owning collection of native particle bindings used by one self-field solve. */
    class ParticleSetView {
    public:
        ParticleSetView(std::span<ParticleFieldBinding3d> bindings, std::size_t primaryIndex);
        ...
        /** @brief Apply an algorithm's selection policy without changing binding membership. */
