...
namespace opalx::spacecharge {
    ...
    CartesianDomainUpdater::CartesianDomainUpdater(CartesianPICConfig config)
    ...
    void CartesianDomainUpdater::updateForSolve(
        ...
        const FieldStorage::Extents extents            = targetExtents(correction);
        const bool repeatFieldLayoutRefresh            = poissonRebuildRequired_m;
        if (fieldStorage.layoutExtents() != extents) {
            // Mark the Poisson solver dirty before mutating layout-owned fields. If a later
            // migration throws, the recovery update must still rebuild it after layouts agree.
            poissonRebuildRequired_m = true;
        }
        const bool layoutChanged = fieldStorage.domain().rebuildGlobalLayoutInPlace(
        ...
        if (layoutChanged || repeatFieldLayoutRefresh) {
            ...
            // Repeating every field refresh is safe and repairs field storage before migration.
            fieldStorage.updateFieldLayoutsAfterLayoutChange();
        }

        CartesianBounds bounds = particles.computeBounds();
        ...
        if (beamFrame && isRedistributionDue(step)
    ...
    bool CartesianDomainUpdater::redistribute(
        ...
        // ORB may mutate the layout before returning or throwing. Keep the Poisson solver dirty so
        // a later update repeats every field refresh before migration and solver reconstruction.
        poissonRebuildRequired_m = true;
