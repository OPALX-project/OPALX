    ...
    if (method == "THIN") {
...
#ifdef HAVE_ENVELOPE_SOLVER
        // Define BET Output files
        FILE* BetOutputDat;
        FILE* BetOutputSli;

        BetOutputDat = fopen("Bop.dat","w");
        BetOutputSli = fopen("Bop.sli","w"); 
        ...
        if (!OPAL.hasSLBunchAllocated()) {
            if (!OPAL.inRestartRun()) {
                ...
                *gmsg << "BET PART STARTED  V 1.0" << endl;
                ...
                *gmsg << "gamma = " << gam << endl; 
                ...
                string fn1("op.sli");
                string fn2("op.dat");	
                string fn3("bunch.dat");
                /*
                   Track::block->slbunch->BetOut(BetOutputDat,BetOutputSli);

                   Track::block->slbunch->writeSlicedBetBunch(fn1.c_str()); 
                   Track::block->slbunch->writeBetStat(fn2.c_str()); 
                   Track::block->slbunch->writeBetBunch(fn3.c_str()); 
                */
                *gmsg << "BET PART FINISHED" << endl;
            } else {
        ...
        */
        ...
        ParallelSliceTracker* tracker2 = new ParallelSliceTracker(*Track::block->use->fetchLine(),
                dynamic_cast<SLPartBunch&>(*Track::block->slbunch), *slds, Track::block->reference,
                false, false,Track::block->maxTSteps, Track::block->zstop); 
        *gmsg << "Initialisation complete" << endl;

        /*** execute parallel slice tracker 
        */

        tracker2->execute(BetOutputDat,BetOutputSli);
        ...
        */
        ...
        fclose(BetOutputDat);
        fclose(BetOutputSli);
        *gmsg << "execute complete" << endl;

        //FIXME: keeps opal from crashing at end of simulation
        //execute is called again (below)
        return;
	...
	if (Options::autoPhase) {
		...
		*gmsg << "* ********************************************************************************** " << endl;
		itsTracker = new AutoPhase(*Track::block->use->fetchLine(), 
					   ...
					   Track::block->zstop, Track::block->bunch->getTrackStep(), Track::block->bunch->getT());
	    }
	}
	else
