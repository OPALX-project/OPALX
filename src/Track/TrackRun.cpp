...
void TrackRun::execute() {
	...
	if (!mpacflg) {// if multipacting flag is not set, do normal initialization of primary particles from attached distribution  
	    dist = Distribution::find(Attributes::getString(itsAttr[DISTRIBUTION]));
	...
	charge = beam->getCharge() * beam->getCurrent() / beam->getFrequency();
		...
		if (!mpacflg) {	
		    ...
		    */
		    ...
		    if(rmin(2) < 0.0) {
			Track::block->bunch->R(2) += abs(rmin(2));
	...
	if(!OPAL.hasBunchAllocated() || Options::scan) {
		...
		*gmsg <<"Multipacting flag is true. Distributions in Run command, if there are any, will be ignored "<<endl;
	    }
	}
