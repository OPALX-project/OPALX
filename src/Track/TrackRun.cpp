...
namespace {
  ...
  enum {
    ...
    FIELDSOLVER,  // The filed solver attached
	 ...
	 "the given lattice.")
{
  itsAttr[METHOD] = Attributes::makeString
    ("METHOD", "Name of tracking algorithm to use:\n"
               "\t\t\t\"THIN\" (default) or \"THICK,PARALLEL-T,PARALLEL-Z\".", "THIN");
  itsAttr[TURNS] = Attributes::makeReal
    ("TURNS", "Number of turns to be tracked; Number of neighboure bunches to be tracked in cyclotron", 1.0);
...
void TrackRun::execute()
{
  ...
  if (method == "THIN") {
    ...
    *gmsg << "* ********************************************************************************** " << endl;
    *gmsg << "  Selected Tracking Method == PARALLEL-T" << endl;
    *gmsg << "* ********************************************************************************** " << endl;

    Beam         *beam = Beam::find(Attributes::getString(itsAttr[BEAM]));

    FieldSolver  *fs   = FieldSolver::find(Attributes::getString(itsAttr[FIELDSOLVER]));    
    fs->initCartesianFields();
    Track::block->bunch.setSolver(*fs);    
    Track::block->bunch.setBCAllOpen();

    Distribution *dist = Distribution::find(Attributes::getString(itsAttr[DISTRIBUTION]));
    ...
    *gmsg <<"Total charge in a bunch ="<<charge*1.0E12<<" [pC]"<<endl;
    ...
    if (!OPAL.inRestartRun()) {
      ...
      dist->doRestart_cycl(Track::block->bunch, beam->getNumberOfParticles(), OPAL.getRestartStep());
      charge /= Track::block->bunch.getTotalNum();
    }
    ...
    *gmsg << "Phase space dump frequency is set to " << Options::psDumpFreq<<endl
    ...
    int numBunch = int(Round(Attributes::getReal(itsAttr[TURNS])));
    *gmsg <<"Number of tracked neighbour bunches is set to "<<numBunch<<endl;

    itsTracker->setNumBunch(numBunch);
