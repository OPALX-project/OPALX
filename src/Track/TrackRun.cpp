...
void TrackRun::execute()
{
    ...
    if (method == "THIN") {
        ...
        *gmsg << "Phase space dump frequency is set to once every " << Options::psDumpFreq << " time steps." << endl
              << "Statistics dump frequency is set to once every " << Options::statDumpFreq << " time steps." << endl
              << "Input file name: " << OPAL.getInputFn() << endl;

        itsTracker = new ParallelTTracker(*Track::block->use->fetchLine(), dynamic_cast<PartBunch&>(*Track::block->bunch), *ds,
