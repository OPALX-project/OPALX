                           ...
                           BMultipoleField &errorField)
{
  if (errorField.order() != 0) {
    ...
    ///JMJ 30/11/2000 added harmless semi-colons in OPAL8 output
    if (Options::opal8) {
      os << "SELECT, ERROR, CLEAR;\nSELECT, ERROR, RANGE = "
         << name << "[" << occur << "], CLASS = " << name
         << ";\nEFCOMP, RADIUS = " << setw(16) << rad << ", ORDER = " << ord;
      double norm = factor * errorField.getNormalComponent(1);
      double skew = factor * errorField.getSkewComponent(1);
      if (norm != 0.0 || skew != 0.0) {
        os << ", &\n  DBLNR     = " << setw(16) << norm
           << ", DBLSR     = " << setw(16) << skew;
      }
      for (int comp = 1; comp < errorField.order(); ++comp) {
        double factor = relFactor(ord, comp, rad) / base;
        norm = factor * errorField.getNormalComponent(comp + 1);
        skew = factor * errorField.getSkewComponent(comp + 1);
        if (norm != 0.0 || skew != 0.0) {
          os << ", &\n  DKLNR("
             << setw(2) << comp << ") = " << setw(16) << norm
             << ", DKLSR("
             << setw(2) << comp << ") = " << setw(16) << skew;
        }
      }
      os << ';' << endl;
    } else {
