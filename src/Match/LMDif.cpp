...
void LMDif::execute()
{
  ...
  if (! Match::block->evaluate(x, f)) {
	...
	if ((abs(actred) <= tol && prered <= tol && ratio <= 2.0) ||
	    (delta <= tol*dxnorm) ||
	    (fnorm*fnorm <= tol)) {
	  state = CONVERGED;
	} else if (Match::block->getCallCount() >= nfcnmax) {
	  state = CALL_LIMIT;
	} else if ((abs(actred) <= DBL_EPSILON && prered <= DBL_EPSILON &&
		    ratio <= 2.0) ||
...
bool LMDif::findJacobian(Vector<double> &x, Vector<double> &f)
{
  ...
  for (int j = 0; j < n; ++j) {
    double step = (temp_x(j) == 0) ? fraction : fraction*abs(temp_x(j));
    temp_x(j) += step;
