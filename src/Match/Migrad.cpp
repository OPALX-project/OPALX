    ...
    if(lambda >= 1.0  &&
       Match::block->evaluate(x + 0.5 * lambda * s, f)) {
        ...
        for(int npts = 2; npts <= maxpoints;) {
            ...
            // Begin iteration.
            lambda_max = max(lambda_max, alpha * abs(lambda_opt));
            ...
            while(true) {
                ...
                for(int i = 0; i < 3; ++i) {
                    if(abs(lambda - lambda_val[i]) < tol9) break;
                }
...
int Migrad::symmetricEigen(const Matrix<double> &V, Vector<double> &eigen) {
    ...
    for(int l = 0; l < n; ++l) {
        ...
        int m;
        for(m = 0; m < n - 1; ++m) if(abs(work(m)) <= b) break;

        if(m != l) {
            for(int iter = 1; iter <= itmax; ++iter) {
                double p = (eigen(l + 1) - eigen(l)) / (2.0 * work(l));
                double r = (abs(p) > 1.0e10) ? abs(p) : sqrt(p * p + 1.0);
