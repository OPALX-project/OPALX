...
public:
    ...
    KOKKOS_INLINE_FUNCTION static void computeField(
            ...
            const ViewType& Br, double hr, double hz, double zbegin, int num_gridpr,
            int num_gridpz) {
        ...
        if ((indexz < 0) || (indexz + 2 > num_gridpz)) return;

        if (indexr + 2 > num_gridpr) return;
        ...
        double BfieldR = (1.0 - leverz) * (1.0 - leverr) * Br(index1)
                         ...
                         + (1.0 - leverz) * leverr * Br(index2) + leverz * leverr * Br(index2 + 1);

        double BfieldZ = (1.0 - leverz) * (1.0 - leverr) * Bz(index1)
                         + leverz * (1.0 - leverr) * Bz(index1 + 1)
                         + (1.0 - leverz) * leverr * Bz(index2) + leverz * leverr * Bz(index2 + 1);
