...
public:
    ...
    // virtual bool getFieldDerivative(const Vector_t &R, Vector_t &E, Vector_t &B, const
    // DiffDirection &dir) const;
    ...
    KOKKOS_INLINE_FUNCTION static void computeField(
            const Vector_t<double, 3>& R, Vector_t<double, 3>& E, Vector_t<double, 3>& B,
            const ViewType& Ez, const ViewType& Er, const ViewType& Bt, double hr_m, double hz_m,
            double zbegin_m, int num_gridpr_m, int num_gridpz_m) {
        ...
        if ((indexz < 0) || (indexz + 2 > num_gridpz_m)) return;

        if (indexr + 2 > num_gridpr_m) return;
...
private:
    ...
    Kokkos::DualView<double*> FieldstrengthEz_m; /**< 2D array with Ez, read in first along z0 - r0
                                                    to rN then z1 - r0 to rN until zN - r0 to rN  */
