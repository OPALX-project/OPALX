...
void OpalParser::parseAssign(Statement &stat) const
{
  ...
  static Object *boolConstant   = OPAL.find("BOOL_CONSTANT");
  static Object *realConstant   = OPAL.find("REAL_CONSTANT");
  static Object *realVariable   = OPAL.find("REAL_VARIABLE");
  static Object *realVector     = OPAL.find("REAL_VECTOR");
  static Object *stringConstant = OPAL.find("STRING_CONSTANT");
  ...
  if (stat.delimiter("->")) {
    ...
    } else if ((object = OPAL.find(objName)) == 0) {
      throw ParseError("OpalParser::parseAssign()",
		       "The object \"" + objName + "\" is unknown.");
    }
  } else {
    // Assignment to variable-like object.
    if ((object = OPAL.find(objName)) == 0) {
      Object *model = 0;
      switch (code) {
      case 0x01:  // CONSTANT
...
void OpalParser::parseMacro(const string &macName, Statement &stat) const
{
  ...
  if (stat.delimiter(':')) {
    ...
    if (Object *macro = OPAL.find(className)) {
    ...
    // Macro call.
    if (Object *macro = OPAL.find(macName)) {
      // Backtrack to first argument.
      stat.restore();
      Object* instance = 0;
      try {
