  ...
  void SequenceWriter::execute()
    {
      ...
      if (Options::opal8) {
        //JMJ 30/11/2000 added harmless semi-colon in OPAL8 output
        itsStream << "   MARKER,AT=" << sum_length << ";\nENDSEQUENCE"
                  << std::endl;
      } else {
  ...
  void SequenceWriter::applyDefault(const ElementBase &element)
    {
      ...
      if (objectName[0] != '#') {
        ...
        //JMJ 30/11/2000 allowed harmless semi-colon in OPAL8 output
        // if (! Options::opal8)
        itsStream << ";\n";
...
void MakeSequence::execute()
{
  ...
  OPAL.apply(MakeSequenceNS::ObjectFlagger());

  // Write all variables.
  os << comment << line << '\n'
     << comment << "Variable definitions." << '\n'
     << comment << line << '\n';
  OPAL.apply(MakeSequenceNS::VariableWriter(os));
  os << '\n';
  ...
  os << comment << line << '\n'
     ...
     << comment << line << '\n';
  OPAL.apply(MakeSequenceNS::ElementWriter(os));
  os << std::endl;
