...
enum class Status {
    Disabled, Initialized, Advanced, Crossing, InvalidInput, DecreasingTime, TurnOverflow
};
...
KOKKOS_INLINE_FUNCTION bool finite(const Endpoint& endpoint) {
    ...
    for (unsigned d = 0; d < 3; ++d)
        if (!Kokkos::isfinite(endpoint.position(d))
            || !Kokkos::isfinite(endpoint.momentum(d))) return false;
    return true;
}
} // namespace detail
 ...
 */
KOKKOS_INLINE_FUNCTION Status update(
        const Plane& plane, State& state, const Endpoint& accepted, Sample& sample,
        bool enabled = true) {
    if (!enabled) return Status::Disabled;
    if (!detail::finite(accepted) || !Kokkos::isfinite(plane.tolerance)
        || plane.tolerance <= 0) return Status::InvalidInput;
    for (unsigned d = 0; d < 3; ++d)
    ...
    if (state.armed && before < 0 && after >= 0) {
        ...
        for (unsigned d = 0; d < 3; ++d) {
            candidate.crossing.position(d) = state.previous.position(d)
                + candidate.fraction * (accepted.position(d) - state.previous.position(d));
            candidate.crossing.momentum(d) = state.previous.momentum(d)
    ...
    state.previous = accepted;
    if (after < -plane.tolerance) state.armed = true;
    else if (after >= 0) state.armed = false;
