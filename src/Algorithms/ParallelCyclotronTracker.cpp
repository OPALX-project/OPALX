...
void ParallelCyclotronTracker::execute(){
  ...
  // time interval between bunch. Unit: ns  for multi-bunch simulation.
  double timeInterval = 0.0;

  // parameters and flag for synchronized injection for multi-bunch simulation.
  double timeStepForSynch = dt;
  bool   needSynch = true;
  ...
  *gmsg<<"-----------------------------"<<endl;
  ...
  if (numBunch_m > 1){
    ...
    *gmsg << "Time interval between neighbour bunches is set to "<<timeInterval<<"[ns]"<<endl;
    ...
    timeStepForSynch = fmod( timeInterval, dt );
    if ( timeStepForSynch == 0.0 ) needSynch = false;
    if ( needSynch ) *gmsg << "time step after injecting new bunch is set to "<<timeStepForSynch<<"[ns]"<<endl;
  }
  ...
  *gmsg<< "Begining time of this run is "<<t << " [ns]"<<endl;
  ...
  *gmsg<<"particles repartition frequency is set to "<<RepartitionFreq<<endl;
  ...
  // flag to change step time for synchronize the time of multple bunches
  // only for one track step after backup dump step and after inject new bunch steps
  bool needChangeStepTime = false;
      
  // time point determine the next time point of check for transition 
  double timeNextCheck = t + timeInterval;
  ...
  for(step; step < maxSteps_m; step++) 
  {
    ...
    if (initialTotalNum > 2)
    {
      ...
      if (numBunch_m > 1 ){
        ...
        if( (BunchCount == 1) && (multiBunchMode_m == 2) && (!flagTransition))
        {
	      if ( t >= timeNextCheck ){
            ...
            *gmsg<<"checking for automatically injecting new bunch ..." << endl;
            ...
            if ( (RThisTurn - RLastTurn) < CoeffDBunches_m * XYrms )  
            {
              ...
              // synchronize 2rd bunch
              needChangeStepTime = true;
        ...
        }else if ( (BunchCount < numBunch_m) && (t >= timeNextCheck) ) {
	      ...
	      *gmsg<<"time = "<<t<<" [ns], Now inject a new bunch by reading data from h5 file ... ... ..."<<endl;
          ...
          // synchronize the next bunch
          if ( BunchCount < numBunch_m ) needChangeStepTime = true;
        ...
        if ( needSynch ){
          ...
          // restore normal time step
          if (dt  == timeStepForSynch ){
            needChangeStepTime = false;
            dt = itsBunch->getdT()*1.0e9; 
          }

          // change time step for this track step 
          if (needChangeStepTime == true){
            dt = timeStepForSynch;
            needChangeStepTime = false;
          }
        }
      ...
      if(itsBunch->hasFieldSolver() && initialTotalNum >=1000)
      {
        ...
        if ( !(itsBunch->weHaveBins()) && step > 10 ) // avoid dump at the first step
          if((oldReferenceTheta < initialReferenceTheta) && (temp_meanTheta >= initialReferenceTheta )){
...
//           Ippl::Comm->barrier();
      ...
      if ( step > 10 && (oldReferenceTheta < initialReferenceTheta) && (temp_meanTheta >= initialReferenceTheta) ){
    ...
    if ( ( ( (step+1) % Options::psDumpFreq == 0 ) && initialTotalNum != 2) || (doDumpAfterEachTurn && dumpEachTurn && initialTotalNum != 2) )
    {
      ...
      if ( (step+1)%(Options::psDumpFreq) == 0)
      {
