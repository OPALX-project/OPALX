...
void ParallelCyclotronTracker::execute(){
    ...
    // temporal 3 real space varibles of particle ID=0~NoSamplePart, [x,y,z]. for tune with SC.  Unit: mm
    // todo: only processor 0 need this matrix. 
    double varible_tuneSamParts_X[NoSamplePart];
    double varible_tuneSamParts_Y[NoSamplePart];
    double varible_tuneSamParts_Z[NoSamplePart];
    // int TestId[NoSamplePart]; //todo: delete
    ...
    *gmsg<< "Time step is set to "<<dt << " [ns]"<<endl;
	...
	outfTheta0<<"#  r [mm]          p_r[rad]       theta [mm]          p_theta[rad]        z [mm]          p_z[rad]"<<endl;
    }
	...
	string  SfileName2 = SfileName+string("-Angle1.dat");

	outfTheta1.precision(8);
	outfTheta1.setf(ios::scientific, ios::floatfield);

	outfTheta1.open(SfileName2.c_str());
	outfTheta1<<"#  r [mm]          p_r[rad]       theta [mm]          p_theta[rad]        z [mm]          p_z[rad]"<<endl;
    }
	...
	string  SfileName2 = SfileName+string("-Angle2.dat");

	outfTheta2.precision(8);
	outfTheta2.setf(ios::scientific, ios::floatfield);

	outfTheta2.open(SfileName2.c_str());
	outfTheta2<<"#  r [mm]          p_r[rad]       theta [mm]          p_theta[rad]        z [mm]          p_z[rad]"<<endl;
    }

    ofstream outfThetaEachTurn;
	...
	outfThetaEachTurn.precision(8);
	outfThetaEachTurn.setf(ios::scientific, ios::floatfield);

	outfThetaEachTurn.open(SfileName2.c_str());
	outfThetaEachTurn<<"#  r [mm]          p_r[rad]       theta [mm]          p_theta[rad]        z [mm]          p_z[rad]"<<endl;
    }
	...
	*gmsg<<"particles energy bin ID reset frequency is set to "<<resetBinFreq<<endl;
    ...
    double deltaTheta = pi/(stepsPerTurn);
    ...
    for(step; step < maxSteps_m; step++)
	{
	    ...
	    if (initialTotalNum > 2)
		{
			...
			if (!flagDoTune) {
				...
				int counter = 0;	 
				int counterAll = 0;
				for(int ii=0; ii<NoSamplePart; ii++) found[ii] = -1;
				...
				*gmsg<<"Found "<< counterAll<<" sample particles."<<endl;
				...
				if(myN == 0) {
				    ...
				    *gmsg<<"Finish receiving!"<<endl;
				    ...
				    for ( itId = tmpi.begin(); itId != tmpi.end(); itId++ ){
					varible_tuneSamParts_X[*itId]=*itParameter;
					itParameter = itParameter+2;
					varible_tuneSamParts_Y[*itId]=*itParameter;
					itParameter = itParameter+2;
					varible_tuneSamParts_Z[*itId]=*itParameter;
					itParameter = itParameter+2;
				    }
					...
					double tempx, tempy;
					double tempDTheta;
					varible_tune0[0]=varible_tuneSamParts_X[ii];
					varible_tune0[1]=varible_tuneSamParts_Y[ii];
					varible_tune0[2]=varible_tuneSamParts_Z[ii];
					...
					// *gmsg<< "ID="<<ii<<", minDTheta = "<<minDTheta*180/pi<<"[deg]"<<endl;

					r_tuning[0] = tempx*cos(tempTheta)+tempy*sin(tempTheta);
	    	    
					// define vector<double> TdeltrSC[NoSamplePart],TdeltzSC[NoSamplePart]
	         
					r_tuning[1] = varible_tuneSamParts_X[ii]*cos(tempTheta)+varible_tuneSamParts_Y[ii]*sin(tempTheta);
					z_tuning[1] = varible_tuneSamParts_Z[ii];
	      
					TdeltrSC[ii].push_back(r_tuning[1]  - r_tuning[0]); 
					TdeltzSC[ii].push_back(z_tuning[1]);
					//debug
					if (ii >= NoSamplePart-10 ){
					    *gmsg<<"partID = "<<ii;
					    *gmsg<<" dR = "<<r_tuning[1]  - r_tuning[0];
					    *gmsg<<" dZ = "<<z_tuning[1]<<endl;
					}
					//end debug
				    }
	      
				    Ttime.push_back((t-StartTime)*1.0e-9);
				    TturnNumber.push_back(turnnumber-StartTurnnumber);
				    *gmsg<<"Time = "<<t<<", turn number = "<<turnnumber<<endl;
				    *gmsg<<"Acount time = "<<t-StartTime <<", turn number = "<< turnnumber-StartTurnnumber<<endl; 
				}else {
				    // for other nodes
				    Message* smsg = new Message();
				    smsg->put(counter);
		...
		double temp_meanTheta = calculateAngle2(varible[0], varible[1]);//[ -pi ~ pi ]
		//if ( step > 10 && (temp_meanTheta > initialReferenceTheta-deltaTheta) && (temp_meanTheta < initialReferenceTheta+deltaTheta) ){
		if((oldReferenceTheta < initialReferenceTheta-deltaTheta) && (temp_meanTheta >= initialReferenceTheta-deltaTheta )){
		    turnnumber++;
		    dumpEachTurn = true;
		    outfThetaEachTurn<< "#Turn number = "<<turnnumber<<", Time = "<<t<<" [ns]"<<endl;
		    outfThetaEachTurn<<" "<<sqrt(varible[0]*varible[0]+varible[1]*varible[1])
				     <<" "<<varible[3]*cos(temp_meanTheta)+varible[4]*sin(temp_meanTheta)
				     <<" "<<temp_meanTheta/pi*180
				     <<" "<<-varible[3]*sin(temp_meanTheta)+varible[4]*cos(temp_meanTheta)
				     <<" "<<varible[2]
				     <<" "<<varible[5]<<endl;
		}
		...
		if((oldReferenceTheta < azimuth_angle0-deltaTheta) && (temp_meanTheta >= azimuth_angle0-deltaTheta )){
		    outfTheta0<< "#Turn number = "<<turnnumber<<", Time = "<<t<<" [ns]"<<endl;
		    outfTheta0<<" "<<sqrt(varible[0]*varible[0]+varible[1]*varible[1])
			      <<" "<<varible[3]*cos(temp_meanTheta)+varible[4]*sin(temp_meanTheta)
			      <<" "<<temp_meanTheta/pi*180
			      <<" "<<-varible[3]*sin(temp_meanTheta)+varible[4]*cos(temp_meanTheta)
			      <<" "<<varible[2]
		...
		if((oldReferenceTheta < azimuth_angle1-deltaTheta) && (temp_meanTheta >= azimuth_angle1-deltaTheta )){
		    outfTheta1<< "#Turn number = "<<turnnumber<<", Time = "<<t<<" [ns]"<<endl;
		    outfTheta1<<" "<<sqrt(varible[0]*varible[0]+varible[1]*varible[1])
			      <<" "<<varible[3]*cos(temp_meanTheta)+varible[4]*sin(temp_meanTheta)
			      <<" "<<temp_meanTheta/pi*180
			      <<" "<<-varible[3]*sin(temp_meanTheta)+varible[4]*cos(temp_meanTheta)
			      <<" "<<varible[2]
		...
		if((oldReferenceTheta < azimuth_angle2-deltaTheta) && (temp_meanTheta >= azimuth_angle2-deltaTheta )){
		    outfTheta2<< "#Turn number = "<<turnnumber<<", Time = "<<t<<" [ns]"<<endl;
		    outfTheta2<<" "<<sqrt(varible[0]*varible[0]+varible[1]*varible[1])
			      <<" "<<varible[3]*cos(temp_meanTheta)+varible[4]*sin(temp_meanTheta)
			      <<" "<<temp_meanTheta/pi*180
			      <<" "<<-varible[3]*sin(temp_meanTheta)+varible[4]*cos(temp_meanTheta)
			      <<" "<<varible[2]
	    ...
	    if ( ( ( (step+1) % Options::psDumpFreq == 0 ) && initialTotalNum != 2) || (doDumpAfterEachTurn && dumpEachTurn && initialTotalNum != 2) )
		{
		    ...
		    *gmsg<<"meanR=( "<<meanR(0)<<" "<<meanR(1)<<" "<<meanR(2)<<" ) [mm] "<<endl;
			...
			{
			    ( ((*DumpSindex)->second).second )->apply(meanR, Vector_t(0.0), t, extE, extB);
			}
	...
	*gmsg << "********** The result for tuning calulation (NO space charge) ********** "<<endl
	...
	if(myN == 0 && flagDoTune) {
		...
		*gmsg<<" average dZ = "<<dZ_average<<endl;
		...
		tuneflag = gettuning(Ttime,TdeltrSC[ii], TdeltzSC[ii], TturnNumber.back(),nur,nuz);
		*gmsg<< "tuneflag = "<<tuneflag<<endl;
		*gmsg<< "++++++++++++++++++++"<<endl;
	    }
	}
    }
