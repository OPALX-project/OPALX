...
const double chtmc_p = PCHARGE/(PMASS*c); // s*C/kg*m
const double chtm_p = PCHARGE/PMASS; // C/kg 
...
void ParallelCyclotronTracker::visitCyclotron(const Cyclotron &cycl)
{
  ...
  msg<<"So total reference momentum   = "<<referencePtot*1000.0<<" [MCU]"<<endl;
...
void ParallelCyclotronTracker::execute(){
  ...
  double dt = itsBunch->getdT()*1.0e9; //[s]-->[ns]
  int q = -1;
  double clightTOgamma;
  long long step, restartStep0 = 0;
  long long psDumpStep = 0;
  double varible[6];//[x,y,z,px,py,pz]
  ...
  double timeInterval=0.0; // time interval between bunch. Unit: ns
  // record how many bunched has already been injected.
  int BunchCount = itsBunch->getNumBunch();  

  *gmsg << "executing ParallelCyclotronTracker( 4 order Runge-Kutta Algorithm )" << endl;

  if(OPAL.inRestartRun()){
    
    psDumpStep = OPAL.getRestartStep();
    restartStep0 = itsBunch->getTrackStep();
    *gmsg <<"Restart at integration step "<<restartStep0<<endl;
    *gmsg <<"The existing bunch number is "<<BunchCount<<endl;
  }
  ...
  size_t initialLocalNum = itsBunch->getLocalNum(); // initial Particles Number on native node. 
  size_t initialTotalNum = itsBunch->getTotalNum();
  int myN = Ippl::myNode();
  ...
  *gmsg<<"-----------------------------"<<endl;
  ...
  if (numBunch > 1){
    ...
    double harm = (((*sindex)->second).second )-> getCyclHarm(); 
    double RfFreq = (((*sindex)->second).second )-> getRfFrequ(); // [MHz]
    timeInterval = harm / RfFreq *1000.0; // [ns]
    *gmsg << "Time interval between neighbour bunches is set to "<<timeInterval<<"[ns]"<<endl;
  }
  
  
  //*****************I***************
  // transform initial coordinates and momenta from local beam frame (relative value to the barycenter )
  ...
  if (initialTotalNum == 1){
    ...
    outfTheta0<<"# ID   x [mm]          px[rad]       y [mm]          py[rad]        z [mm]          pz[rad]"<<endl;
  }
 ...
 if(!OPAL.inRestartRun())
  {
    ...
    Vector_t dumpExtField[2];
    for(int k=0; k<2; k++) dumpExtField[k] = Vector_t(0.0,0.0,0.0);  
   
    // dump the initial distribution
    itsBunch->R /= Vector_t(1000.0); // mm --> m
    itsDataSink->writePhaseSpace_cycl(*itsBunch, dumpExtField);
    itsBunch->R *= Vector_t(1000.0); // m --> mm
    *gmsg <<"phase space dumping at initial position "<<endl;
    *gmsg << "Dump step = 0"<<endl;
    /*
    ...
    */
    ...
    if ((initialTotalNum > 2 ) && ( numBunch > 1) ){
    ...
    // todo: currently force to set numBunch = 1.
    // The initial distribution should be read from that of step =0 in h5 file    
  }
  
  //  set dump frequency of single particle trajectory  
  int SinglePartDumpFreq = Options::sptDumpFreq;
  int RepartitionFreq = Options::repartFreq;
  ...
  *gmsg<<"particles repartition frequency is set to "<<RepartitionFreq<<endl;
  ...
  int boundpDestroyFreq=SinglePartDumpFreq*100;
  ...
  int turnnumber=0, lastTurn=0;
  double oldReferenceTheta = 0.0;
  bool flagNoDeletion;

  if(initialTotalNum == 1){
    *gmsg << "***---------------------------- SINGLE PARTICLE MODE------ ----------------------------*** "<<endl;
    *gmsg << "Instruction: when the totoal particle number equal to 1, single particle mode is triggered automatically," <<endl
          << " The in initial distribution file must be specified which should contain only one line for the single particle "<<endl
          << "and the second line is offcenter particle. The tuning is calculated by FFT routines based on the two particles. "<<endl
    ...
    *gmsg << "Instruction: when the totoal particle number equal to 2, SEO mode is triggered automatically." <<endl
          << "This mode does NOT include any RF cavities. The in initial distribution file must be specified"<<endl
  ...
  for(step; step < maxSteps_m; step++) 
  {
    ...
    if (initialTotalNum > 2)
    {
      ...
      //*****************(II-1-2)***************
       ...
       if (numBunch !=1 ){
         if(!OPAL.inRestartRun()){
           if ( (BunchCount < numBunch ) && (t >( timeInterval * BunchCount) ) ){

             BunchCount++;
             int oldLocalNUM = itsBunch->getLocalNum();
             int newLocalNum = initialLocalNum + oldLocalNUM;
             itsBunch->create(initialLocalNum);
             for (int ii=oldLocalNUM; ii<newLocalNum; ii++){

               itsBunch->R[ii] = initialR[ii-oldLocalNUM];
               itsBunch->P[ii] = initialP[ii-oldLocalNUM];
               itsBunch->Bin[ii] = BunchCount;
      ...
      if(itsBunch->hasFieldSolver() && initialTotalNum >=1000)
      {
        ...
        double temp_meanTheta = calculateAngle(temp_meanR(0), temp_meanR(1));
        if ( oldReferenceTheta > temp_meanTheta ){
	...
	//        *gmsg<<"mean angle      : "<<temp_meanTheta*180.0/pi<<" degree"<<endl;
        ...
        */
        ...
        if( step%RepartitionFreq == 0 ) itsBunch->do_binaryRepart();
	...
	//        *gmsg<<"mean angle      : "<<temp_meanTheta*180.0/pi<<" degree"<<endl;
      ...
      for (int i = 0; i < ( itsBunch->getLocalNum()); i++) {
        ...
        if (!flagNoDeletion){
          *gmsgAll <<"Warning: particle with ID="<<itsBunch->ID[i]<<" is lost at "<< step <<"st step!"<<endl;
          // delete this particle now.
        ...
        for ( beamline_list::iterator sindex = ++(FieldDimensions.begin()); sindex != FieldDimensions.end(); sindex++)
        {
          ...
          if(tag_crossing){
            ...
            if (nomalRadius < 0 || nomalRadius > 1) {
              ...
              (dynamic_cast<RFCavity*>(((*sindex)->second).second)) -> getMomentaKick(nomalRadius, tempP, t, dt1); // t : ns
      ...
      if(!itsBunch->hasFieldSolver() && flagNeedUpdate) 
      { itsBunch->update();
      
      *gmsg <<"After some particles lost, the total particles number is : "<< itsBunch->getTotalNum()<<endl;
      ...
      if ( (int(t/dt)%freqSample) == 0 ){
        ...
        Tdeltz.push_back(z_tuning [1] - z_tuning[0]);
      ...
      double temp_meanTheta = calculateAngle(varible[0], varible[1]);
        if ( oldReferenceTheta > temp_meanTheta ){
    ...
    {
      IpplTimings::startTimer(DumpTimer_m);
      *gmsg <<"phase space dumping after step "<<step+1<<endl;
      *gmsg << "Dump step = "<<( ++psDumpStep )<<endl;
      
      itsBunch->setTrackStep((step+1));
      
      //------------------calculate mean coordinates and momunta of bunch------------------//
      // TODO: should be changed in parallel enviroment using reduce(). Should be changed for multi-bunch simulation.
      ...
      meanR /= Vector_t(( itsBunch->getLocalNum())); 
      meanP /= Vector_t(( itsBunch->getLocalNum()));
      
      *gmsg<<"meanR =("<<meanR(0)<<", "<<meanR(1)<<", "<<meanR(2)<<") [mm] "<<endl;

      double meanP2 = pow(meanP(0),2.0) + pow(meanP(1),2.0) + pow(meanP(2),2.0) ;
      double meanEnergy =(sqrt(1.0 +meanP2)-1.0)*m_p*1000.0; // mean energy on a node for proton, unit: MeV

      // the mean propagation angle of bunch in global Cartesian frame.
      double meanPhi = calculateAngle(meanP(0), meanP(1));
      // the angle from positive radial direction to propagational  direction
      // ==>
      // the angle from positive azimuthal direction to propagational direction
      meanPhi -= pi/2.0;

      double meanPLongitudinal = sqrt( meanP(0)*meanP(0) +  meanP(1)*meanP(1) ); 
      double meanRadius = sqrt( meanR(0)*meanR(0) +  meanR(1)*meanR(1) );
      

      //------------calculate the external field at the mass of bunch------------------------//
      ...
      {
        ( ((*DumpSindex)->second).second )->getFieldstrength(meanR, t, extE, extB);
      } 
      ...
      itsBunch->R /= Vector_t(1000.0); // mm --> m
      itsDataSink->writePhaseSpace_cycl(*itsBunch, FDext );
      itsBunch->R *= Vector_t(1000.0); // m --> mm
      // *gmsg << "Step " << step << ", t = " << itsBunch->getdT()*step*1.0e9 << " [ns]" << endl;
  ...
  *gmsg << "***---------------------------- PARTICLES TRACK DONE------ ----------------------------*** "<<endl;
  ...
  for(int ii=0; ii<( itsBunch->getLocalNum()); ii++) {
    if (itsBunch->ID[ii] == 0) {         
      ...
      *gmsgAll << "Final Energy E of reference particle = "<< FinalEnergy<<" [MeV]"<<endl;
       *gmsg << "Total phase space dump number(includes the initial distribution) = "<< psDumpStep+1 <<endl;
       *gmsg << "One can restart simulation from the last dump step ( -restart "<< psDumpStep <<" )"<<endl;
    }
  }
  ...
  if (initialTotalNum == 2){
    *gmsg << "turn number is "<< lastTurn <<endl;

    bool tuneflag = false;
    double nur,nuz;
    tuneflag = gettuning(Ttime,Tdeltr,Tdeltz,lastTurn,nur,nuz);
    
  }else{

    *gmsg << "Total finished turn number = "<< turnnumber <<endl;
  }
...
{    
  ...
  {
    ( ((*sindex)->second).second )->getFieldstrength(tempR, t, externalE, externalB);
  }else {
    return false;
  }
  ...
  */
  ...
  yp[3] = (externalE(0)*chtmc_p  + (externalB(2)*y[4] - externalB(1)*y[5])*chtm_p/tempgamma )*1.0e-9; // [1/ns]
  yp[4] = (externalE(1)*chtmc_p  - (externalB(2)*y[3] - externalB(0)*y[5])*chtm_p/tempgamma )*1.0e-9; // [1/ns];
  yp[5] = (externalE(2)*chtmc_p  + (externalB(1)*y[3] - externalB(0)*y[4])*chtm_p/tempgamma )*1.0e-9; // [1/ns];
...
double ParallelCyclotronTracker::calculateAngle(double x, double y)
{
 ...
 void ParallelCyclotronTracker::readInitialDist(int BeamCount)
{
  ...
  if(Ippl::myNode() == Ippl::getNodes() - 1)
    endi = -1;//<------------- have problems for serial modes
  else
    endi = starti + numberOfParticlesPerNode;

  //  *gmsg << "Node " << Ippl::myNode() << " gets particles " << starti << " - " << endi << endl;

  H5PartSetView(H5file,starti,endi);
  N = (int)H5PartGetNumParticles(H5file);

  *gmsg << "Node " << Ippl::myNode() << " gets " << N << " initial particles" << endl;
