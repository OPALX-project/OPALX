  ...
  *gmsg << "***---------------------------- Start tracking   ------------------------------------------*** "<<endl;
  for(step; step < maxSteps_m; step++)
  {
	...
	if (!flagDoTune) {
	  ...
	  // TODO: for tune calculation of 100 sample particles, changes need be done to
	  // implement this function: node0 receive and store the coordinates
	  // of the 100 particles. 
	  ///////////////////////////////////////////////////////////////
	  
	  // for all nodes, find the location of particle with ID = 0 & 1~100 in bunch containers
	  ...
	  if(myN == 0) {
	    ...
	    *gmsgAll<<"Finish receiving!"<<endl;
	    ...
	    for ( itId = tmpi.begin(); itId != tmpi.end(); itId++ ){
	      if (*itId != 0){
		varible_tuneSamParts_X[(*itId)-1]=*itParameter;
		itParameter = itParameter+2;
		varible_tuneSamParts_Y[(*itId)-1]=*itParameter;
		itParameter = itParameter+2;
		varible_tuneSamParts_Z[(*itId)-1]=*itParameter;
		itParameter = itParameter+2;
		//TestId[(*itId)-1]=*itId;
	      }else{
