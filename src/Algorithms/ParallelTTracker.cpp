...
void ParallelTTracker::execute()
{
    ...
    *gmsg << "Track start at: " << myt1.time() << " t= " << itsBunch->getT() << endl;
    ...
    *gmsg << "executing ParallelTTracker, initial DT " << itsBunch->getdT()
    ...
    for (int i = 0; i < itsBunch->getLocalNum(); ++i) {
        unsigned int &l = itsBunch->LastSection[i];
    ...
    if (!OPAL.hasBunchAllocated()) {
        updateSpaceOrientation(true);  // vec{p} = (0,0,p_z), vec{r} = (0,0,z)
    } else {
        updateSpaceOrientation(false); // vec{p} = (0,0,p_z), vec{r} = (x,y,z)
    }
    ...
    */
    ...
    if (rep) {
        repartFreq = (int)rep->getReal();  // this variable controls the minimal number of steps until we repartition the particles 
    ...
    for (step; step < maxSteps_m; ++step) {
        ...
        */
        ...
        for (unsigned int i = 0; i < itsBunch->getLocalNum(); ++i) {
            ...
            unsigned int ls = itsBunch->LastSection[i];
            ...
            const unsigned int rtv = itsOpalBeamline_m.getFieldAt(i, itsBunch->R[i], ls, t+itsBunch->dt[i]/2., externalE, externalB);
        ...
        if (hasWake) {
            ...
            wf = itsOpalBeamline_m.getWake(wfSection);
        } else if (wakestatus) {
        ...
        */
        ...
        if (totalParticles_f > 0) {
            if (!bends) { 
                ...
                updateReferenceParticle();
                      ...
                      * in z direction
                      */
                Vector_t tmpP_suv = RefPartP_suv_m;
                ...
                const unsigned int& rtv = itsOpalBeamline_m.getFieldAt(RefPartR_suv_m, t, externalE, externalB);
                bool RefPartinBend = (rtv & BEAMLINE_BEND);
                ...
                if (RefPartinBend) {
                    ...
                    tmpP_suv = Vector_t(0.0, 0.0, sqrt(dot(RefPartP_suv_m, RefPartP_suv_m)));
                }
        ...
        if ( totalParticles_f > 0) {
            ...
            if (totalParticles_f <= minBinEmitted) {
                *gmsg << myt2.time() << " Step " << step << "; only " << totalParticles_f << " particles emitted; t= " << t << " [s]" << endl;
                ...
                *gmsg << myt2.time() << " Step " << step << " at " << sposRef << " [m] t= " << t << " [s]" << endl;
        ...
        if(step > emissionSteps) {
            ...
            reduce(&global_EOL, &global_EOL + 1, &global_EOL, OpBitwiseOrAssign());
            if (global_EOL)
                break;
        }
    }
