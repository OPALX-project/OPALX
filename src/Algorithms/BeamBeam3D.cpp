...
void BeamBeam3D::synchroBeamCollision(FVps<double,6> &map) const
{
  ...
  for ( ; slice != last_slice; ++slice) {
    ...
    if (abs(sx[0] - sy[0]) < 1.0e-6 * abs(sx[0] + sy[0])) {
  ...
  const
{
  ...
  double fac2 = 2.0 * abs(sigxx - sigyy);
  double fac  = sqrt(fac2);
  double sigxy = sqrt(sigxx/sigyy);
  double expfac = 0.0;
  complex<double> w1 =
    (*errorFunction)(complex<double>(abs(sepx), abs(sepy)) / fac);

  if(x < 100.0) {
    expfac = exp(- x*0.5);
    w1 -= expfac * (*errorFunction)
      (complex<double>(abs(sepx)/sigxy, abs(sepy)*sigxy) / fac);
  }
