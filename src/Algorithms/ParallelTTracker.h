...
inline void ParallelTTracker::updateSpaceOrientation(const bool& move)
{
    ...
    itsOpalBeamline_m.updateOrientation(EulerAngles);
...
inline void ParallelTTracker::writePhaseSpace(const long long step, const double &sposRef)
{
    ...
    for(int k = 0; k < 3; ++k)
        {
            ...
            itsOpalBeamline_m.getFieldAt(pos[k], itsBunch->getT() - 0.5 * itsBunch->getdT(), externalE, externalB);
    ...
    if (step % Options::psDumpFreq == 0) {
        ...
        *gmsg << "* Wrote beam phase space." << endl;
        *gmsg << *itsBunch << endl;
    }
