...
getflag:				/* Collect the flag characters: */
    switch( c = *fmt++ ) {
     case '#':	psp->PSflags |= PRF_ALT;	goto getflag;
     case '-':	psp->PSflags |= PRF_MINUS;	goto getflag;
     case '+':	psp->PSflags |= PRF_PLUS;	goto getflag;
     case ' ':	psp->PSflags |= PRF_BLANK;	goto getflag;
     default:	break;
    }
    if( c == '0' ) {
