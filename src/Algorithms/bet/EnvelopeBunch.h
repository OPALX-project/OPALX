...
public:
    ...
    void initialize(int sli, double charge, double energy, double width, double te, double frac,
                    double current, double center, double bX, double bY, double mX, double mY, double Bz);
     ...
     */
    void run(double tStep, double zCat = 0.0);
    ...
    /// set KR value for slice i
    void setKR(Vector_t value, int i) { this->KR[i] = value; }
    /// set KT value for slice i
    void setKT(Vector_t value, int i) { this->KT[i] = value; }
    /// set extern electric field for slice i
    void setEF(Vector_t value, int i) { this->EF[i] = value; }
    /// set extern magnetic field for slice i
    void setBF(Vector_t value, int i) { this->BF[i] = value; }
...
private:
    ...
    inline double *vector(int nn) {
        double *r;
        r = (double *) malloc(sizeof(double) * nn);
        if(!r)
            throw OpalException("EnvelopeBunch", "EnvelopeBunch::vector - Could not allocate memory");
        return r;
    }
    ...
    /**
     * @brief create a bunch with slices
     *
     * @param nSlice will be created
     */
     ...
     */
    void setLShape(EnvelopeBunchShape shape, double z0, double w, double frac);
