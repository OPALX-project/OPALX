...
/* allocate a double matrix with subscript range [0..n1-1][0..n2-1] */
{
  ...
  if (!m) {
    //writeError(errModeAll,errFatal,"Allocation failure1 matrix(%d,%d) in svdfit.C\n",n1,n2);
    writeError();
  }
  ...
  for (i=0; i < n1; i++) {
    ...
    if (!m[i]) {
      //writeError(errModeAll,errFatal,"Allocation failure2 matrix(%d,%d) in svdfit.C at i=%d\n",n1,n2,i);
      writeError();
    }
  }
  m[n1] = NULL;
...
static void svdcmp(double **a, int m, int n, double w[], double **v)
{
  ...
  for (k=n;k>=1;k--) {
	...
	break;
      ...
      if (its == 30) {
	//writeError(errModeAll,errFatal,"svdfit(): no convergence in 30 svdcmp iterations");
 	writeError();
      } 
      x=w[l];
      nm=k-1;
