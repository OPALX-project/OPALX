...
static double *vector(int n) {
  ...
  if (!b) {
    //writeError(1,errModeAll,errFatal,"Insufficient memory malloc %d bytes (rk.C)",sizeof(double)*n);
    writeError();
  }
  return b;
} /* vector */
...
/* allocate a double matrix with subscript range [0..n1-1][0..n2-1] */
{
  ...
  if (!m) {
    //  writeError(1,errModeAll,errFatal,"Allocation failure1 matrix(%d,%d) in rk.C\n",n1,n2);
    writeError();    
  }
  ...
  for (i=0; i < n1; i++) {
    ...
    if (!m[i]) {
      // writeError(1,errModeAll,errFatal,"Allocation failure2 matrix(%d,%d) in rk.C at i=%d\n",n1,n2,i);
    writeError();
    }
  }
  m[n1] = NULL;
...
static void rkSetBuffer(int nvar) {
  if (nvar < 1) {
    //writeError(1,errModeAll,errFatal,"odeint nvar (%d) < 1 (impossible)",nvar);
    writeError();
  }
  if (kmax > 0) {
    if (xp) free(xp);
 ...
 void (*derivs)(double, double [], double []))
{
  ...
  for (;;) {
    ...
    if (errmax > 1.0) {
      ...
      xnew=(*x)+h;
      if (xnew == *x) {
	//writeError(1,errModeAll,errWarning,
 ...
 void (*derivs)(double, double [], double []))
{
  ...
  *nok = (*nbad) = kount = 0;
  ...
  for (nstp=0;nstp<MAXSTP;nstp++) {
    ...
    if (fabs(hnext) <= hmin) {
      // writeError(1,errModeAll,errWarning,"Step size too small in odeint (%le < %le)",
      //	 hnext,hmin);
      writeError();
      return 1;
    }
  ...
  //writeError(1,errModeAll,errWarning,"Too many steps in routine odeint (%d)",MAXSTP);
    writeError();
  return 1;
 } /* odeint */
