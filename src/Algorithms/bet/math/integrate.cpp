...
static double *vector(int n) {
    ...
    if(!b) {
        //writeBetError(errModeAll,errFatal,"Insufficient memory malloc %d bytes (rk.C)",sizeof(double)*n);
        writeBetError();
    }
    return b;
} /* vector */
    ...
    for(m = 1; m < n; m++) {
        for(i = 1; i <= n - m; i++) {
            ...
            if((den = ho - hp) == 0.0) {
                ...
                //writeBetError(errModeAll,errFatal,"Error in routine polint (singular point)");
                writeBetError();
            }
            den = w / den;
            d[i] = hp * den;
