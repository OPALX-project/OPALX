 ...
 double xacc)                               // accuracy
{
  ...
  if ((fl > 0.0 && fh > 0.0) || (fl < 0.0 && fh < 0.0)) {
    //writeError(errModeAll,errFatal,"Root must be bracketed in findRoot()");
    writeError();
  }
  if (fl == 0.0) return x1;
  if (fh == 0.0) return x2;
  ...
  //writeError(errModeAll,errFatal,"Maximum number of iterations (%d) exceeded in findRoot()",MAXIT);
  writeError();
