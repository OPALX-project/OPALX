...
void splint(double xa[], double ya[], double y2a[], int n, double x, double *y)
{
  ...
  if (xb < xa[0]) {
    xb = xa[0];
    //writeError(errModeAll,errGeneral,"slint called with x < xMin: %lf < %lf",x,xa[0]);
    writeError();
  }
  if (xb > xa[n-1]) {
    xb = xa[n-1];
    //writeError(errModeAll,errGeneral,"slint called with x > xMax: %lf > %lf",x,xa[n-1]);
    writeError();
  }
...
void lsplint(double xa[], double ya[], double y2a[],int n, double x, double *y)
{
  ...
  if (xb < xa[0]) {
    xb = xa[0];
    //writeError(errModeAll,errGeneral,"lsplint called with x < xMin: %lf < %lf",x,xa[0]);
    writeError();
  }
  if (xb > xa[n-1]) {
    xb = xa[n-1];
    //writeError(errModeAll,errGeneral,"lsplint called with x > xMax: %lf > %lf",x,xa[n-1]);
    writeError();
  }
