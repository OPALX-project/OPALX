...
Profile::Profile(double *_x,double *_y,int _n)
{
  ...
  if ((x == NULL) || (y == NULL)) 
    //writeError(errModeAll,errFatal,"Insuffient memory for Profile(n=%d)",n);
    writeError();
...
Profile::Profile(char *fname,double eps) {
  ...
  f = fopen(fname,"r");
  if (!f) {
    //writeError(errModeAll,errFatal,"Cannot load profile mapping (%s)",fname);
    writeError();
  }
  i = 0;
  m = 0.0;
...
void Profile::create()
{
  ...
  if (y2 == NULL) { 
    //writeError(errModeAll,errFatal,"Insuffient memory for Profile y2 (n=%d)",n);
    writeError();
  }
  for (i=0; i<n; i++) {
...
void Profile::dump(char *fname,double dx) {
  ...
  if (f) {
    ...
    //writeError(errModeAll,errWarning,"ERROR: Failed to create profile output: %s",fname);
    writeError();
  }
}
