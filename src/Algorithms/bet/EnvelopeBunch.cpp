...
void EnvelopeBunch::create(int N) {
    ...
    if(LastSection) delete[] LastSection;
    if(dt) delete[] dt;
...
//some empty)
void EnvelopeBunch::setBinnedLShape(EnvelopeBunchShape shape, double z0, double w, double frac) {
    ...
    for(int i = 0; i < bins_m.size(); i++) {
        ...
        for(int j = 0; j < bins_m[i].size(); j++)
            cout << " " << mySliceStartOffset_m + bins_m[i][j];
        cout << endl;
    }

    //find first bin containing slices
    firstBinWithValue_m = 0;
    for(firstBinWithValue_m; firstBinWithValue_m < nebin_m; firstBinWithValue_m++)
...
void EnvelopeBunch::synchronizeSlices() {
    ...
    MPI_Allreduce(MPI_IN_PLACE, &(z_m[0]), numSlices_m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &(b_m[0]), numSlices_m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
}
...
void EnvelopeBunch::calcI() {
    ...
    for(int i = 0; i < numSlices_m; i++) {
        if(b_m[i] > 0.0) {
            if(i == mySliceStartOffset_m)
                my_start = n1;
            if(i == mySliceEndOffset_m)
                my_end = n1;
...
#pragma omp parallel for private(i,j,dz)
    ...
    MPI_Allreduce(MPI_IN_PLACE, &(I1[0]), n1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    //MPI_Allreduce(MPI_IN_PLACE, &(z2_temp[0]), n1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    //MPI_Allreduce(MPI_IN_PLACE, &(I2_temp[0]), n1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
void EnvelopeBunch::cSpaceCharge() {
    ...
    int nVTot = nV;
    MPI_Allreduce(MPI_IN_PLACE, &nVTot, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    if(nVTot < 2) {
        free(xi);
    ...
    MPI_Allreduce(MPI_IN_PLACE, &xi[0], numSlices_m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &sm, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    A0 = sm / nVTot;
...
#ifdef USE_HOMDYN_SC_MODEL
        double kpc  = 0.5 * Physics::c * Physics::c * (Y[SLI_beta] * Physics::c) * activeSlices_m * Q_m / numSlices_m / (zHead() - zTail()) / Physics::Ia;
...
void EnvelopeBunch::run(double tStep, double _zCat) {
    ...
    if(false && lastEmittedBin_m < nebin_m) {
        ...
        if(bins_m[nextBin].size() == 0 && nextBin > firstBinWithValue_m)
            iStart = numMySlices_m;
        else if(bins_m[nextBin].size() != 0)
            iStart = bins_m[nextBin][0] - 1;
...
void EnvelopeBunch::initialize(int sli, double Q, double energy, double width, double te, double frac, double current, double center, double bX, double bY, double mX, double mY, double Bz0, int nbin) {

    //FIXME: WHY?
    if(sli < 30 * Ippl::Comm->getNodes()) {
        sli = 30 * Ippl::Comm->getNodes();
        *gmsg << "WARNING: increasing number of slices to " << sli << endl;
    }
    createSlices(sli);
    ...
    *gmsg << "Using BET space-charge model" << endl;
    ...
    lastEmittedBin_m = 0;
    activeSlices_m = 0;
    tEmission_m = te / nebin_m;
    this->setBinnedLShape(i2 ? bsGauss : bsRect, center, te, frac);
