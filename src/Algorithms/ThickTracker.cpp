...
void ThickTracker::visitMultipole(const Multipole &mult)
{
  ...
  if (length == 0.0) {
    ...
    for (int i = 0; i < itsBunch.getLocalNum(); i++) {
      Particle part = itsBunch.get_part(i);
      Vector zin;
      zin[X] = part.x();  zin[PX] = part.px();
      zin[Y] = part.y();  zin[PY] = part.py();
      zin[T] = part.t();  zin[PT] = part.pt();
      ...
      if (zin[X]!=DBL_MAX) {
        ...
        try {
          ...
          part.x() = zf[X];  part.px() = zf[PX];
          part.y() = zf[Y];  part.py() = zf[PY];
          part.t() = zf[T];  part.pt() = zf[PT];
        ...
        if (!ok) {
          part.x() = DBL_MAX;  part.px() = DBL_MAX;
          part.y() = DBL_MAX;  part.py() = DBL_MAX;
          part.t() = DBL_MAX;  part.pt() = DBL_MAX;
        }
      }
      itsBunch.set_part(part,i);
    }
				      ...
				      const BMultipoleField &field, double scale)
{
  ...
  for (int i = 0; i < itsBunch.getLocalNum(); i++) {
    ...
    double x = part.x();  double px = part.px();
    double y = part.y();  double py = part.py();
    double t = part.t();  double pt = part.pt();
    if (x!=DBL_MAX) {
      double p=pt+1.0;
  ...
  while(abs(st)<as){
    ...
    try{
      ...
      if(++ci>cx) {
        ...
        throw ConvergenceError("ThickTracker::implicitInt2()",msg);
      }
      if (!cnvWarn) {
        std::cerr << " <***WARNING***> [ThickTracker::implicitInt2()]:\n"
