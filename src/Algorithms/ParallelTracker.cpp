 ...
 */
void ParallelTracker::computeSpaceChargeFields(unsigned long long step, OrbitThreader& oth) {
...
std::optional<BEAMBEAM::ActualGeometry> ParallelTracker::detectBeamBeamWindow(
    ...
    try {
        elements = oth.query(bunchS + 0.5 * (rmax[2] + rmin[2]), rmax[2] - rmin[2]);
    ...
    for (const auto& element : elements) {
        ...
        const double beamBeamWindowLength = element->getElementLength();
...
void ParallelTracker::logBeamBeamDiagnostics(bool force) {
    ...
    if (beamBeamState_m.geometry.has_value()
        ...
        for (const size_t ci : beamBeamState_m.geometry->config.witnessContainers) {
            ...
            witnessStates << "c" << ci << ":" << (active ? "active" : "inactive") << ":n=" << total;
        }
    }
