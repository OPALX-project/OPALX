...
pair<FieldList::iterator, bool>  AutoPhase::doOneStep(BorisPusher pusher)
{
    ...
    } else { /* at least one of the elements bends the beam; until all particles 
    ...
    for (; step_m < maxSteps_m; ++step_m) {
        ...
        if (res.second) {
            ...
            *gmsg << "Start phase scan ... " << endl;
            ...
            for (int j = 0; j < 4; ++ j) { // 4 == levels of refinement; 
                // why 4? together with the 9 from below we get a resolution 
                // of less than 1 degree (0.43896 degrees to be precise)
                double dphi = (phi_end - phi_ini) / 9.; // 9 == number of sampling points 
                // between phi_ini and phi_end. why 9? see above.
                vector<PhaseEnT> e;
            ...
            itsBunch->R[0] = xi;
            itsBunch->P[0] = pi;
            maxphases << (*res.first).getElement()->getName() << "_phi= "  << Phimax << "; // E= " << Emax << " (MeV)" << endl;
