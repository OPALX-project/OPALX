...
void Period::doomSummary(DoomWriter &writer) const
{
  // Size of summary record.
  writer.putInt(numColumns + 2, 5);

  // Delta(p) = 0.0.
  writer.putReal(0, 0.0);

  // Tunes.
  writer.putReal(3, Attributes::getReal(itsAttr[Q1]));
  writer.putReal(4, Attributes::getReal(itsAttr[Q2]));
}
