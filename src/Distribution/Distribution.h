...
enum DistrTypeT {GAUSS,BINOMIAL,UNIFORMXYZ,FROMFILE,GUNGAUSSFLATTOPTH,GUNUNIFORM,GUNGAUSS3D,NODIST};
...
public:
    ...
    void createSlicedBunch(double charge, double gamma, double mass,double current, double center, double Bz0, SLPartBunch* p);
#endif
...
private:
    ...
    //void createTimeBins(const int Np, const double sigma, const unsigned int bins, const double trise, const double tfall, 
    //const double temis, gsl_histogram * h) ;
    void createTimeBins(const int Np, const double sigma_rise, const double sigma_fall, const unsigned int bins, 
