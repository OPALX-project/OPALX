...
Distribution::Distribution():
  Definition(SIZE,"DISTRIBUTION","The DISTRIBUTION statement defines data for the 6D particle distr.")
  {
    itsAttr[DISTRIBUTION] = makeString("DISTRIBUTION", "Distribution type: GAUSS, BINOMIAL, ROTSYMBINOMIAL, FROMFILE,"
        "GUNGAUSS, GUNGAUSS3D, GUNUNIFORM, GUNGAUSSFLATTOP, UNIFORMXYZ ", "GAUSS");
...
void Distribution::create(PartBunch &beam, size_t Np) {
  ...
  if (isBinned)
	  ...
	  if (scan_m) 
		  ...
		  beam.update();
		  INFOMSG("In scan more: deleted BIN structure and all particles in the bunch" << endl;);
	      }
      }
  else
      pbin_m = NULL;
  ...
  if (disttype == "GUNGAUSS" || disttype == "GUNUNIFORM" || disttype == "GUNGAUSS3D" || disttype == "GUNGAUSSFLATTOP")
...
{
  const double &two_pi = Physics::two_pi;  unsigned int pc = 0;
  ...
  *gmsg << "* Gamma= " << gamma << " Beta= " << beta << endl;
  ...
  if (pc == Ippl::myNode()) {
    ...
    *gmsg << "* Transverse cut  " << transvCutOff << " [sigma] " << endl;
    double Hs2a = Attributes::getReal(itsAttr[SIGMAX]);
    ...
    for(int i=0; i<Np; i++) {
      ...
      } else if (distType == "GUNGAUSSFLATTOP") {
        if (i < particlesFront) {
          // Fill rise.
          psi0 = -1.0;
          while (psi0 < 0.0)
...
void PartBins::updatePartInBin(size_t countLost[]) {
  ...
  Inform m2all("updatePartInBin ", INFORM_ALL_NODES);

  //  for (int ii=0; ii < bins_m; ii++){
  for (int ii=0; ii < nemittedBins_m; ii++){
    m2all << "In Bin: "<< ii <<", "<< nBin_m[ii] <<" particles "<<endl;
  }
  ...
  for (int ii=0; ii < nemittedBins_m; ii++){
    ...
    if(flagNeedUpdate){
      ...
      m2all << "In Bin: "<< ii <<", "<< countLost[ii] <<" particle(s) lost"<<endl;
    }
  }
}
