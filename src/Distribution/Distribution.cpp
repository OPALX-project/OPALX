...
Distribution::Distribution():
    ...
    itsAttr[DISTRIBUTION] = makeString("DISTRIBUTION", "Distribution type: GAUSS, BINOMIAL, ROTSYMBINOMIAL, FROMFILE,"
                                       "GUNGAUSS, GUNGAUSS3D, GUNUNIFORM, GUNGAUSSFLATTOP, GUNGAUSSFLATTOPTH, UNIFORMXYZ SURFACEEMISSION", "GAUSS");
    ...
    itsAttr[TPULSEFWHM]  = makeReal("TPULSEFWHM", "Pulse FWHM (s)", 1.0e-12);
    itsAttr[TRISE]       = makeReal("TRISE", "Rise time for GUNGAUSSFLATTOP distribution type (s)", 1.0e-12);
    itsAttr[TFALL]       = makeReal("TFALL", "Fall time for GUNGAUSSFLATTOP distribution type (s)", 1.0e-12);
    ...
    itsAttr[WORKFUNCTION] = makeReal("WORKFUNCTION", "Work function of gun surface material (eV)", 4.65);
    itsAttr[FENHANCEMENT] = makeReal("FENHANCEMENT", "Field enhancement factor for Fowler-Nordheim emission", 50.0);
    itsAttr[MAXFNEMISSION] = makeReal("MAXFNEMISSION", "Maximum number of electrons emitted from a single triangle for Fowler-Nordheim emission", 1000.0);
    ...
    workFunction_m = Attributes::getReal(itsAttr[WORKFUNCTION]);
    fieldEnhancement_m = Attributes::getReal(itsAttr[FENHANCEMENT]);
    maxFN_m = (size_t) Attributes::getReal(itsAttr[MAXFNEMISSION]);
}
/**
 *
 *
 ...
 * @param p
 */
void Distribution::createSlicedBunch(double charge, double gamma, double mass, double current, double center, double Bz0, EnvelopeBunch *p) {
    ...
    *gmsg << "beamWidth = " << beamWidth << " beamCenter = " << center << " beamEnergy = " << beamEnergy << endl;
    ...
    p->initialize(sl, charge, beamEnergy, beamWidth, tEmission_m, frac, current, center, Hs2a_m, Vs2a_m, 0, 0, Bz0);
...
double Distribution::getWorkFunction() { return Attributes::getReal(itsAttr[WORKFUNCTION]); }
double Distribution::getFieldEnhancement() { return  Attributes::getReal(itsAttr[FENHANCEMENT]); }
size_t Distribution::getMaxFNemissionPartPerTri() { return (size_t) Attributes::getReal(itsAttr[MAXFNEMISSION]);}
...
Inform &Distribution::print(Inform &os) const {
    ...
    if(!OPAL.inRestartRun()) {
        ...
        if(Attributes::getString(itsAttr[DISTRIBUTION]) == "SURFACEEMISSION") {
            ...
            os << "* Field enhancement for surface emission  " << Attributes::getReal(itsAttr[FENHANCEMENT]) << endl;
            os << "* Maximum number of electrons emitted from a single triangle for Fowler-Nordheim emission  " << Attributes::getReal(itsAttr[MAXFNEMISSION]) << endl;
        } else {
