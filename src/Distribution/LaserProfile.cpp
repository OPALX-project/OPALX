...
void LaserProfile::ReadFile(string fn, string imagestr, double cut) {
    ...
    hid_t gr = H5Gopen(h5, imagestr.c_str());
    int   imageNumber = 1;

    //hid_t dsetBit = H5Dopen(gr,"CameraBits");
    //hid_t dsetBit = H5Dopen(gr,"CameraGain");

    //double bitData[] = { -1 };
    //herr_t status = H5Dread(dsetBit, H5T_IEEE_F64LE,H5S_ALL,H5S_ALL,H5P_DEFAULT,bitData);
    //int  bits = static_cast<int>(bitData[0]);
    int bits = 8;

    hsize_t dim[2];
    double dimData[2] = { -1, -1 };
    hid_t   dset = H5Dopen(gr, "CameraData");
    /*
    hsize_t dim[3];
    double dimData[3] = { -1, -1, -1 };
    hid_t   dset = H5Dopen(gr, "CameraData");
    */
    ...
    */
    ...
    herr_t status = H5Dread(dset, H5T_NATIVE_USHORT, mem, filespace, H5P_DEFAULT, image);
