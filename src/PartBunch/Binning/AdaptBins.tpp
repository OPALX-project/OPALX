...
namespace ParticleBinning {
    ...
    AdaptBins<BunchType, BinningSelector>::getBinConfigHost(
            std::vector<size_type>& binCounts, std::vector<value_type>& binWidths) const {
        ...
        hview_type_g countsHost = globalBinHisto_m.template getHostView<hview_type_g>(
                globalBinHisto_m.getHistogram());
        hwidth_view_type_g widthsHost = globalBinHisto_m.template getHostView<hwidth_view_type_g>(
                globalBinHisto_m.getBinWidths());
    ...
    void AdaptBins<BunchType, BinningSelector>::initLimits() {
        ...
        if (nlocal <= 0) {
            msg << level4 << "Particles in the bunch = " << nlocal
                << ". Overwriting limits manually." << endl;
            ...
            // Sadly this is necessary, since Kokkos seems to have a problem when nlocal == 1, where
            // it does not update localMinMax
            if (nlocal == 1) {
        ...
        msg << level4 << "Initialized limits. Min: " << xMin_m << ", max: " << xMax_m
            << ", binWidth: " << binWidth_m << endl;
    }
    ...
    void AdaptBins<BunchType, BinningSelector>::instantiateHistogram(bool setToZero) {
        ...
        if (setToZero) {
            dview_type device_histo = localBinHisto_m.template getDeviceView<dview_type>(
                    localBinHisto_m.getHistogram());
            Kokkos::deep_copy(device_histo, 0);

            // Sync changed (only has an effect if DualView mode is used and the two memory spaces
            // are different!)
    ...
    AdaptBins<BunchType, BinningSelector>::getBin(
        ...
        // Ensure x is within bounds (clamp it between xMin and xMax --> this is only for bin
        // assignment).
        x += (x < xMin) * (xMin - x) + (x > xMax) * (xMax - x);

        bin_index_type bin = (x - xMin) * binWidthInv;
        return (bin >= numBins) ? (numBins - 1)
                                : bin;  // Clamp to the maximum bin if "out of bounds".
    ...
    void AdaptBins<BunchType, BinningSelector>::assignBinsToParticles() {
        ...
        // Declare the variables locally before the Kokkos::parallel_for (to avoid implicit this
        // capture in Kokkos lambda)
        ...
        Kokkos::parallel_for(
                "assignParticleBinsConst", bunch_m.getLocalNum(),
    ...
    void AdaptBins<BunchType, BinningSelector>::executeInitLocalHistoReduction(
        ...
        // Reduce using an array-reducer object (size needs to be known at compile time for GPU
        // kernels)
        Kokkos::parallel_reduce(
                "initLocalHist", bunch_m.getLocalNum(),
        ...
        Kokkos::parallel_for(
                "finalize_histogram", binCount, KOKKOS_LAMBDA(const bin_index_type& i) {
    ...
    void AdaptBins<BunchType, BinningSelector>::executeInitLocalHistoReductionTeamFor() {
        ...
        using scratch_view_type = Kokkos::View<
                size_type*, Kokkos::DefaultExecutionSpace::scratch_memory_space,
    ...
    void AdaptBins<BunchType, BinningSelector>::initLocalHisto(HistoReductionMode modePreference) {
        ...
        if (mode == HistoReductionMode::HostOnly) {
            ...
            std::visit(
                    [&](auto& reducer_arr) {
                        msg << level4 << "Starting parallel_reduce, array size = "
                            << sizeof(reducer_arr.the_array) / sizeof(reducer_arr.the_array[0])
                            << endl;
                        executeInitLocalHistoReduction(reducer_arr);
                    },
                    to_reduce);
            ...
            msg << level4
                << "No valid execution method defined to initialize local histogram for energy "
                   "binning."
                << endl;
        ...
        localBinHisto_m.sync();  // since all reductions happen on device --> sync changes if
                                 // DualView mode is used
    }
    ...
    void AdaptBins<BunchType, BinningSelector>::initGlobalHistogram() {
        ...
        // on host!
        hview_type localBinHistoHost =
                localBinHisto_m.template getHostView<hview_type>(localBinHisto_m.getHistogram());
        hview_type_g globalBinHistoHost = globalBinHisto_m.template getHostView<hview_type_g>(
                globalBinHisto_m.getHistogram());

        // Note: The allreduce also works when the .data() returns a CUDA space pointer.
        //       However, for some reason, copying manually to host and then allreduce-ing is
        //       faster.
        IpplTimings::startTimer(bAllReduceGlobalHistoT);
        ippl::Comm->allreduce(
                localBinHistoHost.data(), globalBinHistoHost.data(), numBins,
                std::plus<size_type>());
    ...
    void AdaptBins<BunchType, BinningSelector>::genAdaptiveHistogram() {
        var_selector_m.updateDataArr(bunch_m);  // Probably not necessary, since it is called before
                                                // updating particles; but just in case!
        bin_view_type binIndex            = getBinView();
        hindex_transform_type adaptLookup = globalBinHisto_m.mergeBins();

        // This step is currently unnecessary, since mergeBins works on host only. However, just in
        // case, we can keep this if should become available on device as well (there is barely a
        // performance hit, since the lookup array is usually only 128 elements large).
        dindex_transform_type adaptLookupDevice =
                dindex_transform_type("adaptLookupDevice", currentBins_m);
    ...
    void AdaptBins<BunchType, BinningSelector>::sortContainerByBin() {
        ...
        dview_type bin_counts =
                localBinHisto_m.template getDeviceView<dview_type>(localBinHisto_m.getHistogram());

        // Get post sum (already calculated with histogram and saved inside
        // local_bin_histo_post_sum_m), use copy to not modify the original
        ...
        Kokkos::parallel_for(
                "SortIndices", localNumParticles, KOKKOS_LAMBDA(const size_type& i) {
...
// Test is sorting was successful (only in debug mode)
        ...
        if (localNumParticles > 1
            && !viewIsSorted<bin_index_type>(getBinView(), indices, localNumParticles)) {
            Inform msg("AdaptBins");
