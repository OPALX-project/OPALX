...
namespace ParticleBinning {
    ...
    public:
        ...
        virtual value_type getBinConfigHost(
                std::vector<size_type>& binCounts, std::vector<value_type>& binWidths) const = 0;
     ...
     *
     * This class provides functionality to group particles into energy bins, initialize and
     * reinitialize bin structures, and update bin contents using data structures from IPPL and
     * Kokkos. It is optimized for usage on CPU and GPU. It is explained in detail in Alexander
     * Liemen's master thesis and was developed for OPAL-X to improve the performance when solving
     * the electric field in a relativistic particle bunch.
     *
     * @tparam BunchType The type of particle bunch (container) used in the binning process.
     * @tparam BinningSelector A function or functor that selects the variable for binning (based on
     * attributes).
     */
    ...
    public:
        ...
        using h_histo_type_g =
                Histogram<size_type, bin_index_type, value_type, false, Kokkos::HostSpace>;
        ...
        /**
         * @brief Constructs an AdaptBins object with a specified maximum number of bins and a
         * selector.
         *
         * @param bunch Borrowed particle container used for binning.
         * @param var_selector A function or functor that selects the variable for binning (based on
         * attributes).
         ...
         */
        AdaptBins(
                BunchType& bunch, BinningSelector var_selector, bin_index_type maxBins,
            ...
            msg << level4 << "AdaptBins initialized with maxBins = " << maxBins_m
                << ", alpha = " << binningAlpha_m << ", beta = " << binningBeta_m
        ...
        /**
         * @brief Gets the maximum number of bins. Will be used for the fine uniform histogram
         * before merging.
         *
         ...
         *
         * @note The other parameters (limits) are set before this function is called in
         * doFullRebin().
         */
         ...
         * on the the reduction functions in this class. It chooses which function to use based on
         * the HistoReductionMode parameter (defined in ParallelReduceTools.h) or the optimal method
         * based on the current architecture/bin number.
         */
        void initLocalHisto(
                HistoReductionMode modePreference = HistoReductionMode::Standard) override;
         ...
         *
         * This function performs a `Kokkos::parallel_reduce` over the particles in the bunch,
         * incrementing counts in the reduction array `to_reduce` based on the bin index for each
         * particle. After the reduction, the results are copied to the final histogram
         * `localBinHisto`.
         *
         * @tparam ReducerType The type of the reduction object, which should support `the_array`
         * for bin counts.
         ...
         * - Executes a parallel loop to copy the reduced bin counts from `to_reduce` to
         * `localBinHisto`.
         *
         * @note This function uses the Kokkos parallel programming model and assumes that
         * `to_reduce` has a `the_array` member which stores the histogram counts. So far, only
         * `ParticleBinning::ArrayReduction` is implemented in that way (to work together with
         * `Kokkos::Sum` reducer). `the_array` needs to have a known size at compile time. To use it
         * on GPU anyways, ParallelReduceTools.h contains a function `createReductionObject()` to
         * select the reducer object from a `std::variant` of pre-compiled reducer sizes.
         */
         ...
         *
         * @return A view of the global histogram in host space (used for merging/the adaptive
         * histogram).
         */
         ...
         * particle data.
         * @param modePreference The preferred mode for histogram reduction (default is Standard and
         * choses the best method itself).
         */
        void doFullRebin(
                bin_index_type nBins, bool recalculateLimits = true,
                HistoReductionMode modePreference = HistoReductionMode::Standard) override {
         ...
         * It assumes that the DualView (if used) has been properly synchronized and initialized. If
        ...
        size_type getNPartInBin(bin_index_type binIndex, bool global = false) override {
            // shouldn't happen..., "binIndex < 0" unnecessary, since binIndex is usually unsigned;
            // but just in case the type is changed
            if (binIndex < 0 || binIndex >= getCurrentBinCount()) {
                return bunch_m.getTotalNum();
            }
         ...
         * It rearranges a particle indices array into the correct order based on their Bin
         * attribute and verifies the sorting. The function uses a parallelized bin sort.
         *
         * Steps:
         ...
         * index. If no DualView is used, it might need to copy some values to host, which might
         * cause overhead. However, this only happens if the localBinHisto_m is implemented as a
         * device only view, which is not its intended type.
         *
         ...
         * specified bin.
         *
         * @note It returns an iteration policy that can be used together with sortedIndexArr_m
         * inside `scatter()` to only iterate and scatter particles in a specific bin.
         */
         ...
         *
         * This function is used to create an adaptive histogram from a fine global histogram. The
         * fine global histogram (usually 128 bins) has to be created beforehand, but does not
         * necessarily have to be uniform (even though this is recommended). The algorithm works
         * perfectly fine as long as the globalBinHisto_m.binWidths_m field is initialized
         * correctly. If not specified otherwise, it will be initialized either uniformly at
         * initialization, or changed by the `mergeBins()` call or copied from another `BinHisto`
         * instance.
         *
         * It performs the following steps:
         * 1. Merges bins in the global histogram to create an adaptive histogram (see
         * `BinHisto::mergeBins()`).
         * 2. Maps old bin indices to new bin indices using the lookup table returned by
         * `mergeBins()`.
         ...
         *       on host.
         */
        value_type getBinConfigHost(
                std::vector<size_type>& binCounts,
         ...
         */
        void print() override { globalBinHisto_m.printPythonArrays(); }
...
// Check number of GPUs (CUDA/HIP devices)
            ...
            msg << level2 << "CUDA Enabled: Rank " << rank << " sees " << num_gpus
    ...
    private:
        ...
        h_histo_type_g
                globalBinHisto_m;  ///< Global histogram view (over ranks reduced local histograms).
