...
namespace ParticleBinning {
    ...
    /**
     * @class Histogram
     * @brief Template class providing adaptive particle histogram binning with support for Kokkos
     * Views and DualViews.
     *
     * The Histogram class implements a flexible histogram for particle binning. It manages bin
     * counts, bin widths, and post-sums, supporting both uniform and adaptive binning (merging).
     * All operations can be performed on the host or device, as required. The class can be
     * initialized as a Kokkos::DualView or a simple Kokkos::View, should it be necessary to use the
     * host/device views separately. In that case, one needs to make sure that the `sync()` and
     * `modify_...` functions are called correctly to ensure that the data is synchronized. The
     * synchronization is a wrapper around the Kokkos sync functions. More can be found in the
     * `Kokkos::DualView` documentation.
     *
     ...
     * @tparam Properties Additional properties for Kokkos Views/DualViews (e.g. explicit memory
     * spaces).
     */
    template <
    ...
    public:
        ...
        /**
         * @brief Constructor for the Histogram class with a given name, number of bins, and total
         * bin width.
         *
         * @param debug_name The name of the histogram for debugging purposes. Is passed as a name
         * for the Kokkos::...View.
         * @param numBins The number of bins in the histogram. Might change once the merging
         * algorithm is used.
         * @param totalBinWidth The total width of the value range covered by the particles, so
         ...
         *
         * @see copyFields() for more information on how the fields are copied (uses Kokkos shallow
         * copy).
         */
         ...
         * It assumes that the DualView has been properly synchronized and initialized. If the
        ...
        /**
         * @brief Sets the bin widths by copying them from a different Histogram instance (usually
         * neccessary after merging the global histogram to copy the new non-uniform widths to the
         * local histogram).
         *
         * @note This function needs to be templated to allow copying from BinHisto instances that
         * are from a different type than "this". It is neccessary in this context, since
         * localBinHisto_m in AdaptBins is usually a DualView setup, while globalBinHisto_m is
         * modified to use a host-only view.
         */
         ...
         *
         * @note The bin widths are assumed to be constant. Should only be called the first time the
         ...
         *
         * @note Should not be called again after merging bins, since the bin widths won't be
         * uniform.
         */
         ...
         *
         * This function initializes the post-sum by computing the fixed sum on the device view of
         * the post-sum member. If the UseDualView constant is true, it modifies the device view and
         * synchronizes it with the host.
         */
        void initPostSum() {
            IpplTimings::startTimer(bHistogramInitT);
        ...
        /**
         * @brief Returns a `Kokkos::RangePolicy` for iterating over the elements in a specified
         * bin.
         *
         ...
         * @return Kokkos::RangePolicy<> The range policy for iterating over the elements in the
         * specified bin.
         *
         * @note The bounds are needed on host. So if no `DualView` is used or the values are only
        ...
        */
        ...
        /**
         * @brief If a DualView is used, it sets the flag on the view that the device has been
         * modified.
         *
        ...
        /**
         * @brief If a DualView is used, it sets the flag on the view that the host has been
         * modified.
         *
         * @see sync() After this function you might call sync at some point.
         */
         ...
         * flag is set to true. Otherwise, it returns the histogram itself (which might not be on
         * device!).
         *
         * @tparam HistogramType The type of the histogram.
         * @param histo Reference to the histogram.
         * @return The device view of the histogram if `UseDualView` is true, otherwise the
         * histogram itself.
         */
        ...
        /**
         * @brief Merges bins in a histogram to reduce the number of bins while minimizing a cost
         * function.
         *
         * This function performs adaptive binning by merging adjacent bins in a histogram. The
         * merging process is guided by a cost function that evaluates the deviation introduced by
         * merging bins. The goal is to minimize the total cost while reducing the number of bins.
         *
         ...
         * 3. Uses dynamic programming to find the optimal bin merging strategy that minimizes the
         * total cost.
         * 4. Reconstructs the boundaries of the merged bins based on the dynamic programming
         * results.
         ...
         *
         * The cost function used for merging is defined by `adaptiveBinningCostFunction()`, which
         * evaluates the deviation introduced by merging bins based on their counts and widths.
         *
         * @note This function assumes that the histogram has been generated with a sufficiently
         * large number of bins (e.g., 128 bins). If the number of bins is too small (<= 1), merging
         * is skipped.
         *
         * @warning If no feasible merges are found, the function falls back to a no-merge scenario
         * and logs a warning.
         */
    ...
    private:
         ...
         * \f[
         * c[i, j] \overset{!}{=}
         * \underbrace{N_\mathrm{tot}\, \log\left(N_\mathrm{tot}\right)\, \Delta
         * v_\mathrm{tot}}_{\text{Shannon Entropy}}
         * + \underbrace{\alpha \, \Delta v_\mathrm{tot}^2}_{\text{Width Bias}}
         * + \underbrace{\beta \, \left(\Delta v_\mathrm{tot} - \Delta
         * v_\mathrm{bias}\right)^2}_{\text{Deviation Penalty}}
         * + \underbrace{\frac{\Delta v_\mathrm{bias}}{N_\mathrm{tot}} \Theta\left(\Delta
         * v_\mathrm{bias}-N_\mathrm{tot}\right)}_{\text{Sparse Penalty}}
         * \f]
         *
         * Where:
    ...
    private:
        ...
        value_type binningAlpha_m;  /// \brief Alpha parameter for the adaptive binning (merging)
                                    /// cost function.
        value_type binningBeta_m;  /// \brief Beta parameter for the adaptive binning (merging) cost
                                   /// function.
        value_type desiredWidth_m;  /// \brief Desired width for the adaptive binning (merging) cost
                                    /// function.
        ...
        view_type postSum_m;  /// \brief View storing the cumulative sum of bin counts (used in
                              /// sorting, generating range policies).
