...
{
  OpalFilter *filter = dynamic_cast<OpalFilter*>(OPAL.find(name));
  ...
  if (filter_m == 0) {
    ...
    *gmsg << "* **********************************************************************************"<< endl;
    ...
    if (Attributes::getString(itsAttr[TYPE]) == "Savitzky-Golay") {
      ...
      int polynomial_order = abs((int)(Attributes::getReal(itsAttr[POLYORDER])));
      ...
      filter_m = new FixedFFTLowPassFilter(abs((int)(Attributes::getReal(itsAttr[NFREQ]))));
    }
    else if (Attributes::getString(itsAttr[TYPE]) == "RelativeFFTLowPass") {
      filter_m = new RelativeFFTLowPassFilter(fabs(Attributes::getReal(itsAttr[THRESHOLD])));
    }
...
Inform &OpalFilter::print(Inform &os) const
{
  os << "* ************* W A K E ************************************************************ " << endl;
  os << "* FILTER         " << getOpalName() << '\n'
