...
public:
    ...
    unsigned int getFieldAt(const unsigned int&, const Vector_t&, const long&, const double&, Vector_t&, Vector_t&);
    unsigned int getFieldAt(const Vector_t&, const Vector_t&, const double&, Vector_t&, Vector_t&);
...
{
    if (index > -1 && index < sections_m.size()) {
...
{
    if (sindex < sections_m.size() && sindex > -1) {
...
inline void OpalBeamline::updateOrientation(const Vector_t& angle, const Vector_t& centroid, const double& smin, const double& smax)
{
    ...
    for (int i = 0; i < sections_m.size(); ++ i) {
        if (online_secs_m[i]) {
            ...
            Vector_t newAngle = oldAngle + angle;
