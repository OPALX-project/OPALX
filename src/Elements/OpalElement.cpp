...
//   This class groups some ancillary functions for the DOOM interface
...
void OpalElement::doomGet(const DoomReader &reader)
{
  // Fetch the element attributes, using the DOOM index table.
  for (std::vector<Attribute>::iterator i = itsAttr.begin();
       i != itsAttr.end(); i++) {
    int index = DoomDB::getAttributeIndex(i->getName());
    if (index >= 0) {
      i->doomGet(reader, index);
    }
  }
}


void OpalElement::doomPut(DoomWriter &writer) const
{
  // Set the object type name.
  writer.setTypeName("ELEMENT");

  // Store the element attributes, using the DOOM index table.
  for (std::vector<Attribute>::const_iterator i = itsAttr.begin();
       i != itsAttr.end(); i++) {
    int index = DoomDB::getAttributeIndex(i->getName());
    if (index >= 0) {
      i->doomPut(writer, index);
    }
  }
}
...
fillRegisteredAttributes(const ElementBase &base, ValueFlag)
{
  ...
  //  attributeRegistry["WAKEF"]->setString(base.getWakeFieldName());
  
  // attributeRegistry["WGEOM");    
  // attributeRegistry["WPARAM");
  // attributeRegistry["WCOND");
  // attributeRegistry["WMAT");
...
const string OpalElement::getWMaterial() const
{
  const Attribute *attr = findAttribute("WMAT");
  return attr ? Attributes::getString(*attr) : string();
}

const string OpalElement::getWakeGeom() const
{
  const Attribute *attr = findAttribute("WGEOM");
  return attr ? Attributes::getString(*attr) : string();
}

std::vector<double> OpalElement::getWakeParam() const
{
 return Attributes::getRealArray(itsAttr[WPARAM]);
}

const string OpalElement::getWakeConductivity() const
{
  const Attribute *attr = findAttribute("WCOND");
  return attr ? Attributes::getString(*attr) : string();
}
...
void OpalElement::parse(Statement &stat)
{
  while (stat.delimiter(',')) {
    ...
    if (attr == 0) {
      ...
      // size in itsSize, so as to recognise the added attributes.
...
void OpalElement::print(std::ostream &os) const
{
  ...
  if (! Options::mad8 && isShared()) head = "SHARED " + head;
  ...
  // OPAL-8 cannot handle the added attributes.
  int size = itsAttr.size();
  if (Options::mad8) size = itsSize;

  for (int i = 0; i < size; ++i)  {
    if (itsAttr[i]) {
      // OPAL-8 cannot handle the aperture.
      if (! Options::mad8  ||  itsAttr[i].getName() != "APERTURE") {
        itsAttr[i].print(len);
      }
    }
  }
        ...
        // above is executed at least for KICKERS, DRIFTS when making OPAL8 output
  ...
  Element(size, name, help), itsSize(size)
{
  ...
  itsAttr[WAKEF]   = Attributes::makeString
    ("WAKEF", "Defines the wake function: WT,WL,WTL i.e. transverse, longitudinal or both");
  itsAttr[WGEOM]   = Attributes::makeString
    ("WGEOM", "Defines the geometry CYLINDER, RECTANGULAR or SLAC type (SLAC-PUB-10707)");
  itsAttr[WMAT]   = Attributes::makeString
    ("WMAT", "Defines the material of the beam pipe: ALUMINUM or COPPER ");
  itsAttr[WPARAM]  = Attributes::makeRealArray
    ("WPARAM", "Parameters for WT and WL");
  itsAttr[WCOND] = Attributes::makeString
    ("WCOND", "The conductivity of the beam pipe AC or DC");
  ...
  if (first) {
    ...
    registerRealAttribute("WAKEF");
    registerRealAttribute("WGEOM");    
    registerRealAttribute("WPARAM");
    registerRealAttribute("WCOND");
    registerRealAttribute("WMAT");
    first = false;
  }
}
