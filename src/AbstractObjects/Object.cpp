...
void Object::doomGet(const DoomReader &reader)
{
  for (vector<Attribute>::size_type i = 0; i < itsAttr.size(); ++i) {
    itsAttr[i].doomGet(reader, i);
  }
}


void Object::doomPut(DoomWriter &writer) const
{
  // Set the object type name.
  const Object *type = this;
  while (type->getParent()) type = type->getParent();
  writer.setTypeName(type->getOpalName());

  // Store the attributes.
  for (vector<Attribute>::size_type i = 0;
       i < itsAttr.size(); ++i) {
    itsAttr[i].doomPut(writer, i);
  }
}
...
void Object::setDoomTime(double time)
{
  timeStamp = time;
}


double Object::getDoomTime() const
{
  return timeStamp;
}
