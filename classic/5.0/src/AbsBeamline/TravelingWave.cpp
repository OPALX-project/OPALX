...
void TravelingWave::setMisalignment(double x, double y, double z)
{
  dx_m = x;
  dy_m = y;
  dz_m = z;
...
void TravelingWave::getMisalignment(double &x, double &y, double &z) const
{
  x = dx_m;
  y = dy_m;
  z = dz_m;
...
bool TravelingWave::readFieldMap(double &startField, double &endField, double scaleFactor)
{
  using Physics::pi;
  ...
  if (frequency_m != CoreFieldmap_m->getFrequency())
    {
      msg << "************ WARNING ********************************************************" << endl;
  ...
  PeriodLength_m = (zEnd - zBegin) / 2.;
  CellLength_m = PeriodLength_m * Mode_m;
  
  endField = startField + CellLength_m * (NumCells_m + 1);
  startField -= CellLength_m;

  startField_m = startField;
  startCoreField_m = 1.5 * CellLength_m;
  startExitField_m = (NumCells_m + 0.5) * CellLength_m;
  mappedStartExitField_m = startExitField_m - 4.5 * CellLength_m;

  scaleCore_m = scale_m * 2. / sqrt(3.);
  phaseCore1_m = phase_m + pi * Mode_m / 2.;
  phaseCore2_m = phase_m + pi * Mode_m * 1.5;
  phaseExit_m = phase_m + 2 * pi * (NumCells_m - 1) * Mode_m;
  
  return true;
}
  
void TravelingWave::rescaleFieldMap(double scaleFactor)
{
	//IFF: still needed?
...
bool TravelingWave::getFieldstrength(double R[], double t, double E[], double B[]) const
{
...
bool TravelingWave::getFieldstrength(Vector_t R, double t, Vector_t &E, Vector_t &B) const
{
