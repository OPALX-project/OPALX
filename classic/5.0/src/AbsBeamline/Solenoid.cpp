...
void Solenoid::addKR(int i, double t, Vector_t &K) 
{
    ...
    // get B-Field
    bool out_of_bounds = myFieldmap_m->getFieldstrength(tmpA,tmpE,tmpB);
    double bz = scale_m * tmpB(2); 

    double g = RefPartBunch_m->getGamma(i);

    double k = pow(Physics::q_e*bz / (2.0*Physics::m_bet*g), 2.0);
    K += Vector_t(k, k, 0.0);
}
...
void Solenoid::addKT(int i, double t, Vector_t &K) 
{
    ...
    out_of_bounds = myFieldmap_m->getFieldstrength_fdiff(tmpA, tmpE_diff, tmpB_diff, zdir);
    ...
    emg  = Physics::q_e/(g*Physics::m_bet);
