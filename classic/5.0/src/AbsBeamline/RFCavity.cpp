...
void RFCavity::initialise(const PartBunch *bunch, double &startField, double &endField, const double &scaleFactor)
{
  ...
  *gmsg << "RFCavity " << getName() << " using file " ;
  fieldmap_m = Fieldmap::getFieldmap(filename_m, fast_m);
  if (fieldmap_m != NULL)
    {
      fieldmap_m->getInfo(gmsg);
      if ((frequency_m - fieldmap_m->getFrequency()) / frequency_m > 0.0001)
        {
          *gmsg << "************ WARNING ********************************************************" << endl;
          *gmsg << " FREQUENCY IN INPUT FILE DIFFERENT THAN IN FIELD MAP; " << endl;
          *gmsg << frequency_m << " <> " << fieldmap_m->getFrequency() << "; TAKE ON THE LATTER" << endl;
          *gmsg << "*****************************************************************************" << endl;
      ...
      if (hasWake()) {
        ...
        *gmsg << "RFCavity initialising wake function" << endl;
      }
    }
  else
    {
...
void RFCavity::rescaleFieldMap(const double &scaleFactor)
{
  ...
  fieldmap_m->rescale(scaleFactor);
...
void RFCavity::getMomentaKick(const double normalRadius,double momentum[], const double t, const double dtCorrt, const int PID )
{
  ...
  if ( PID == 0){
    ...
    gmsgALL<<"Energy Gain in Gap-crossing dE = "<< (dgam)*m_p*1000.0<<" [MeV]"<<endl;
    gmsgALL<<"Current Energy = "<< (gamma-1.0)*m_p*1000.0<<" [MeV]"<<endl;
  }
