...
void Cyclotron::getdiffs() {
    ...
    int index = idx(i,j);
    *gmsg<<"i="<<i<<", j="<<j<<", index="<<index<<", B="<<Bfield.bfld[index]<<endl;
      }
    }
    */
}
...
void Cyclotron::getFieldFromFile_FFAG(const double &scaleFactor) {
    ...
    */
    ...
    msg << "----------------------------------------------" << endl;
    msg << "   READ IN FFAG FIELD MAP     " << endl;
    msg << "----------------------------------------------" << endl;
    ...
    Bfield.ntetS  = Bfield.ntet + 1;
    msg << "Minimal radius of measured field map: " << BP.rmin << " [mm]" << endl;
    msg << "Maximal radius of measured field map: " << rmax << " [mm]" << endl;
    msg << "Stepsize in radial direction: " << BP.delr << " [mm]" << endl;
    msg << "Minimal angle of measured field map: " << BP.tetmin << " [deg.]" << endl;
    msg << "Maximal angle of measured field map: " << maxtheta << " [deg.]" << endl;

    //if the value is negtive, the actual value is its reciprocal.
    if(BP.dtet < 0.0) BP.dtet = 1.0 / (-BP.dtet);
    msg << "Stepsize in azimuth direction: " << BP.dtet << " [deg.]" << endl;
    msg << "Total grid point along azimuth:  " << Bfield.ntetS << endl;
    msg << "Total grid point along radius: " << Bfield.nrad << endl;
    ...
    fp.close();
    msg << "Field Map read successfully nelem= " << count << endl;
}
...
void Cyclotron::getFieldFromFile_AVFEQ(const double &scaleFactor) {
    ...
    Inform msg("visitCyclotron read field ");
    msg << "----------------------------------------------" << endl;
    msg << "   READ IN AVFEQ CYCLOTRON FIELD MAP     " << endl;
    msg << "----------------------------------------------" << endl;
    ...
    */
    ...
    if((f = fopen(fmapfn_m.c_str(), "r")) == NULL) {
        msg << "Error in Cyclotron::getFieldFromFile_AVFEQ()!" << endl;
        msg << " Cannot open file, please check if it really exists." << endl;
        exit(1);
    }
    ...
    msg << "Maximal angle of measured field map: " << tetmax << " [deg.]" << endl;
    ...
    Bfield.ntot = Bfield.ntetS * Bfield.nrad;
    msg << "Total stored grid point number ( ntetS * nrad ) : " << Bfield.ntot << " ntot-idx= " << ntotidx << endl;
...
void Cyclotron::getFieldFromFile_CYCIAE(const double &scaleFactor) {
    ...
    Inform msg("visitCyclotron read field ");
    msg << "----------------------------------------------" << endl;
    msg << "   READ IN CYCIAE-100 CYCLOTRON FIELD MAP     " << endl;
    msg << "----------------------------------------------" << endl;
