...
public:
  ...
  SliceIterator(T *array, ptrdiff_t stride);
...
public:
  ...
  /// Constructor.
  //  Construct iterator for [b]array[/b], given a [b]stride[/b].
  ConstSliceIterator(const T *array, ptrdiff_t stride);
...
template<class T>
inline SliceIterator<T>::SliceIterator(T *array, ptrdiff_t n):
...
template<class T>
inline SliceIterator<T> &SliceIterator<T>::operator+=(ptrdiff_t n)
{
  cursor += n * stride;
  return *this;
}
...
ConstSliceIterator<T> ConstSliceIterator<T>::operator+(ptrdiff_t n) const
{
  ConstSliceIterator<T> tmp(*this);
  return tmp += n;
}
