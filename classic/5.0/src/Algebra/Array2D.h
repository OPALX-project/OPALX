...
inline T& Array2D<T>::operator()(int r, int c)
{
  if (r < 0  ||  r >= rows  ||  c < 0  ||  c >= ncols()) {
    throw RangeError("Array2d::operator()", "Index out of range.");
  }
...
{
  if (r >= rows) {
    throw RangeError("Array2D::row_begin()", "Row index out of range.");
  }
  return &data[cols*r];
}
...
{
  if (r >= rows) {
    throw RangeError("Array2D::row_end()", "Row index out of range.");
  }
...
{
  if (c >= cols) {
    throw RangeError("Array2D::col_begin()", "Column index out of range.");
  }
...
{
  if (c >= cols) {
    throw RangeError("Array2D::col_end()", "Column index out of range.");
  }
