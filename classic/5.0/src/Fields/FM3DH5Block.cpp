...
void FM3DH5Block::readMap()
{
    if (FieldstrengthEz_m == NULL) {
        ...
        if (file) {
            ...
            long field_size = 0;
            int Nnodes = min(20, Ippl::getNodes());
            ...
            h5err = H5Block3dRead3dVectorField(file, "Efield", FieldstrengthEx_m, FieldstrengthEy_m, FieldstrengthEz_m);
            h5err = H5Block3dRead3dVectorField(file, "Hfield", FieldstrengthHx_m, FieldstrengthHy_m, FieldstrengthHz_m);
            ...
            for (long i = 0; i < num_gridpx_m * num_gridpy_m * num_gridpz_m; ++ i) {
                avrg += FieldstrengthEx_m[i];
            }

            index_x = static_cast<int>(floor(-xbegin_m / hx_m + 0.5));
            index_y = static_cast<int>(floor(-ybegin_m / hy_m + 0.5));
