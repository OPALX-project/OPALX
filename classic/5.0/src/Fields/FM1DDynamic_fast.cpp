...
void FM1DDynamic_fast::readMap() {
    if(onAxisField_m == NULL) {
        ...
        for(int i = 0; i < num_gridpz_m; ++ i) {
            ...
            onAxisField_m[i] = 1e6 * RealValues[num_gridpz_m + i];
        }
        in.close();
        ...
        RealValues[0] *= 1.e6 / (2.*num_gridpz_m * Ez_max);
        onAxisField_m[0] /= Ez_max;
        for(int i = 1; i < accuracy; i++) {
            RealValues[i] *= 1.e6 / (Ez_max * num_gridpz_m);
            onAxisField_m[i] /= Ez_max;
        }
        ...
        for(int i = 0; i < 3; ++i) {
            onAxisInterpolants_m[i+1] = gsl_spline_alloc(gsl_interp_cspline, num_gridpz_m);
            gsl_spline_init(onAxisInterpolants_m[i+1], zvals, higherDerivatives[i], num_gridpz_m);
            onAxisAccel_m[i+1] = gsl_interp_accel_alloc();
        }
...
void FM1DDynamic_fast::getOnaxisEz(vector<pair<double, double> > & F) {
