   ...
   imagFourCoefs_m(NULL)
{
  ...
  if (file.good())
    {
      file >> tmpString >> accuracy_m >> tmpDouble;
...
bool FM1DDynamic::getFieldstrength(Vector_t R, Vector_t &E, Vector_t &B)
{
  if (realFourCoefs_m == NULL) readMap();

  double RR2 = R(0)*R(0) + R(1)*R(1);
  double EfieldZ, EfieldR, BfieldT;

  double kz = two_pi * (R(2) - zbegin_m)/length_m;
  ...
  double f  = -(ezpp  + ez *  xlrep_m * xlrep_m)/16.;
  double fp = -(ezppp + ezp * xlrep_m * xlrep_m)/16.;

  EfieldZ =   ez + 4 * f * RR2;
  EfieldR = -(ezp/2. + fp * RR2);
  BfieldT =  (ez/2. + f * RR2) * xlrep_m / c;
