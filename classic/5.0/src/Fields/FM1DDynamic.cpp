  ...
  if (realFourCoefs_m == NULL)
    {
      ...
      num_gridpzp = (int)floor(++num_gridpz/2.) + 1;
      in >> tmpDouble;
      ...
      RealValues = (double*) fftw_malloc(sizeof(double) * num_gridpz);
      FourCoefs  = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * num_gridpzp);

      for (int i = 0; i < num_gridpz; i++)
        in >> RealValues[i];

      p = fftw_plan_dft_r2c_1d(num_gridpz, RealValues, FourCoefs, FFTW_ESTIMATE);
      fftw_execute(p);

      realFourCoefs_m[0] = FourCoefs[0][0] / num_gridpz;
      for (int i = 1; i < accuracy_m; i++)
        {
          realFourCoefs_m[i] = 2./num_gridpz * FourCoefs[i][0];
          imagFourCoefs_m[i - 1] = 2./num_gridpz * FourCoefs[i][1];
        }
  ...
  for (int l = 1; l < accuracy_m ; l++)
    {
      ...
      ez    +=              ( realFourCoefs_m[l] * coskzl + imagFourCoefs_m[l] * sinkzl); somefactor *= somefactor_base;
      ezp   += somefactor * (-realFourCoefs_m[l] * sinkzl + imagFourCoefs_m[l] * coskzl); somefactor *= somefactor_base;
      ezpp  += somefactor * (-realFourCoefs_m[l] * coskzl - imagFourCoefs_m[l] * sinkzl); somefactor *= somefactor_base;
      ezppp += somefactor * ( realFourCoefs_m[l] * sinkzl - imagFourCoefs_m[l] * coskzl);
    }
