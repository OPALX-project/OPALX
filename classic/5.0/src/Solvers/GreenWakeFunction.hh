...
private:
    ...
    public:
        ...
        double operator()(double k) {
            ...
            switch(acMode_) {
                case 1:
                    Z = (Z0_ / (2 * pi * a_)) * 1.0 / (sqrt(Z0_ * abs(k) / 2) * sqrt(sigma_ / (1.0 - i * Physics::c * k * tau_)) * (i + signK) / k - (i * k * a_) / 2.0);
                    break;
                case 2:
    ...
    void compEnergy(const double K, const double charge, vector<double> lambda, double *OutEnergy);
