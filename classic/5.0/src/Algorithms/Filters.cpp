...
void IlyaPogorelovFilter::apply(vector<double> &LineDensity, vector<double> &firstDerivative, const double &hz)
{
  ...
  for (int i = 0; i < size; ++i)
    firstDerivative[i] = (temp1[i+2] - 8. * temp1[i+3] + 8. * temp1[i+5] - temp1[i+6]) / (12. * hz);
...
{
  srand(time(NULL));
  vector<double> data(1028,0.0);
  vector<double> firstDerivative(1028,0.0);

  SavitzkyGolayFilter TestFilter(33, 16, 16, 2);

  ofstream input("SavGol.in");
  ofstream output("SavGol.out");
  ofstream output2("IlyaPogorelov.out");

  for (int i = 0; i < data.size(); ++i)
    {
      data[i] = exp(-((double)(i-512))*((double)(i-512))/(1<<15)) + 0.02 * sin(6.28318530718*i/64) + 0.01 *(((double)rand())/RAND_MAX - 0.5);
      input << i << "\t" << data[i] << "\t" << ((double)(512-i))/(1<<14) * exp(-((double)(i-512))*((double)(i-512))/(1<<15)) + 0.02 * 6.28318530718 / 64 * cos(6.28318530718*i/64)<< endl;
    }

  TestFilter.apply(data);

  for (int i = 0; i < data.size(); ++i)
    output << i << "\t" << data[i] << endl;
  
  IlyaPogorelovFilter::apply(data,firstDerivative,1.0);
  for (int i = 0; i < data.size(); ++i)
    output2 << i << "\t" << data[i] << "\t" << firstDerivative[i] << endl;
