...
void PartBunch::calcGammas() {
  ...
  for (int i=0; i<emittedBins+1; i++) {
    ...
    const size_t pInBin = (binemitted_m[i]);
...
void PartBunch::computeSelfFields()
{
  ...
  if (fs_m->hasValidSolver()) {
    ...
    //IFF: FOR MG ENLARGE GRID HERE!
    //at the moment we dont have coordinates for pipe midpoint
    if(fs_m->getFieldSolverType() == "MG" || fs_m->getFieldSolverType() == "FFTBOX") {
        ...
        if(xmin > rmin_m[0] || xmax < rmax_m[0]) cout << "DOMAIN TO SMALL: PARTICLES OUTSIDE!" << endl;
        if(ymin > rmin_m[1] || ymax < rmax_m[1]) cout << "DOMAIN TO SMALL: PARTICLES OUTSIDE!" << endl;
    ...
    for(int x = 0; x < nr_m[0]; x++) {
        for(int y = 1; y < nr_m[1]; y++) {
            for(int z = 2; z < nr_m[2]; z++) {

                double xx = (x-nr_m[0]/2)*hr_scaled[0];
                double yy = (y-nr_m[1]/2)*hr_scaled[1];
                double zz = (z-nr_m[2]/2)*hr_scaled[2];
                NDIndex<3> l(Index(x,x),Index(y,y),Index(z,z));
                r = sqrt(xx*xx+yy*yy);
                if(r >= 0.5)
                    rho_m.localElement(l)= 0.0;
                else {
                    rho_m.localElement(l) = (96*(1-4*r)+(1-2*r)*(1-2*r)*(6*r+1)*pi*pi)*(1-2*r)*sin(pi*(zz-0.5));
                }
...
void PartBunch::rotateAbout(const Vector_t &Center, const Vector_t &Momentum)
{
  ...
  double cos0 = Momentum(2)/AbsMomentumProj,
    sin0 = -Momentum(0)/AbsMomentumProj,
    cos1 = AbsMomentumProj/AbsMomentum,
    sin1 = -Momentum(1)/AbsMomentum;
  double T00 = cos0,
    T01 = 0.0,
    T02 = sin0,
    T10 = -sin1 * sin0,
