...
template<class T, int S>
inline FSlice<T, S> &FSlice<T, S>::operator+=(ptrdiff_t n) {
    cursor += n * S;
    return *this;
}
...
template<class T, int S>
inline FConstSlice<T, S> &FConstSlice<T, S>::operator+=(ptrdiff_t n) {
    cursor += n * S;
    return *this;
}
...
FConstSlice<T, S> FConstSlice<T, S>::operator+(ptrdiff_t n) const {
    FConstSlice tmp(*this);
    return tmp += n;
}
