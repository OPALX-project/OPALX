...
FTps<T,N> FTps<T,N>::scaleMonomials(const FTps<T,N> &rhs) const
{
  ...
  int f_min = getMinOrder();   int g_min = rhs.getMinOrder();
  int f_max = getMaxOrder();   int g_max = rhs.getMaxOrder();
...
Array1D<int> FTps<T,N>::getRepOrders() const
{
  ...
  result[0] = itsRep->minOrd; result[1] = itsRep->maxOrd;
  result[2] = itsRep->trcOrd; result[3] = itsRep->allocOrd;
  return result;
}
...
void FTps<T,N>::checkOrders(const string &method, int minOrder, int maxOrder, int &trcOrder)
{
  ...
  if (errF) {
    ...
    if (minOrder == EXACT) std::cerr << "EXACT,"; else std::cerr << minOrder << ",";
    if (maxOrder == EXACT) std::cerr << "EXACT,"; else std::cerr << maxOrder << ",";
    if (trcOrder == EXACT) std::cerr << "EXACT";  else std::cerr << trcOrder;
    std::cerr << ")." << std::endl;
...
FTps<T,N> operator+(const FTps<T,N> &lhs, const FTps<T,N> &rhs)
{
  ...
  f_min = FTps<T,N>::orderStart(f_min);  f_max = FTps<T,N>::orderEnd(std::min(f_max,h_max));
  g_min = FTps<T,N>::orderStart(g_min);  g_max = FTps<T,N>::orderEnd(std::min(g_max,h_max));
  h_min = std::min(f_min,g_min);         h_max = std::max(f_max,g_max);
