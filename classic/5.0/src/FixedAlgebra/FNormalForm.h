  ...
  lambda(), V()
{
  ...
  for (int i = 0; i < 2 * freedom; i += 2) {
    ...
    if (abs(imag(lambda[i])) > tol) {
      c1 = c2 = arg(lambda[i]) / 2.0;
    } else {
      c1 = log(abs(lambda[i])) / 2.0;
      c2 = - c1;
    }
  ...
  for (int omega = 2; omega < maxOrder; omega++) {
	...
	if (abs(imag(lambda[j])) > tol) count += index[j+1];
	factor *= (pow(lambda[j],index[j]) * pow(lambda[j+1],index[j+1]));
      }
      
      if (abs(1.0 - factor) < tol) {
				...
				FMatrix<double,N,N> tmat)
{
  ...
  for (int i = 0; i < n_r; ) {
    int m;
    for (m = i + 1; m < n_r; ++m) {
      if (abs(tlam[i] * tlam[m] - 1.0) < tol) break;
    }
  ...
  for (int i = 0; i < nDim; i += 2) {
    ...
    int k = i;
    for (int j = i; j < N; j += 2) {
      double c = abs(V(i,j)*V(i+1,j+1) - V(i,j+1)*V(i+1,j));
