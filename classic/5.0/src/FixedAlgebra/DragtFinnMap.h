...
factorBerzForestIrwin(const FTps<double,2*N> &HH)
{
  ...
  for (int omega = 3; omega <= maxOrder; omega++) {
	...
	if (abs(imag(mu[j])) > tol) count += index[j+1];
...
orderModes(FMatrix<double,2*N,2*N> &V, FVector<complex<double>,2*N> &mu)
{
  ...
  for (int i = 0; i < n_r; ) {
    ...
    double fact1 = - 1.0 / sqrt(abs(2.0 * pb));
...
template <int N>
ostream &operator<<(ostream &os, const DragtFinnMap<N> &map)
{
