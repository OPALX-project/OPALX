...
\begin{frame}{The PSI Cyclotron Facility- from the Viewpoint of a Numerics Guy} 
  \begin{center}
\vspace{-7mm}
...
%
 ...
 \node [f1pblockr, right of=e42] (e43) { 3D SC \\ Collimation\\\&Particle Matter };
  \node [f1pblockr, right of=e43] (e44) { 3D SC \\ Collimation\\\&Particle Matter };
   \node [f1pblockr, right of=e44] (e45) { 3D SC };
...
\begin{figure}[htb]
   \centering
  \includegraphics[width=0.6\linewidth]{trace3}
\end{figure}
% \end{block}
 ...
 \begin{tikzpicture}[scale=0.8, transform shape]
    \footnotesize
      ...
      \node[fill= green!40] (0_00) at (7.0,0.0) {Distributions};
       ...
       \node[fill=blue!45] (q_20) at (0,-3.5) {Trilinos Interface};
...
\begin{figure}[h]
  \begin{center} 
    \input{figures/Latex-Figures/envelop-1.tex}
%    \caption{Sketch of a sliced bunch}
    \label{fig:shape}
  \end{center} 
\end{figure}
%
...
\begin{figure} \vspace{-3mm}
  \begin{center} 
    \includegraphics[angle=90,width=0.9\linewidth]{/Users/adelmann/svnwork/ineichen/data/et-valid-sc-plots/ET_101-TT-ET_151-ET_501-varepsilon-x-SPOS.pdf} \\
    \includegraphics[angle=90,width=0.5\linewidth]{/Users/adelmann/svnwork/ineichen/data/et-valid-sc-plots/ET_101-TT-ET_151-ET_501-varepsilon-x-SPOS-beginning.pdf}
    \includegraphics[angle=90,width=0.5\linewidth]{/Users/adelmann/svnwork/ineichen/data/et-valid-sc-plots/ET_101-TT-ET_151-ET_501-varepsilon-x-SPOS-end.pdf}
 \caption{Emittance}
  \end{center} 
\end{figure}
\end{frame}

\begin{frame}{\opal within the Simulation Live-Cycle}{Online Modeling: Data Formats, Data Processing}
...
\path [line,->] (OPAL) edge node[left] {\bf H5Part - PARALLEL}   (DISK);
\path [line] (HERONION) -> (OPALGEO);

\path [line] (OPALLAT) -> (OPAL);
\path [line] (OPALGEO) -> (OPAL);

\path [line] (XFEL) -> (EPICS);
\path [line] (EPICS) -> (CA);
\path [line] (EPICS) -> (MCA);
\path [line,<->] (MCA) edge  node[right] {\bf HDF5 (H5Part)}  (DISK);
\path [line,<->] (CA) edge node[left] {\bf H5Part} (DISK);
\end{tikzpicture}
\end{frame}
...
\node [progblock,right of=NULL4](MCA) {MCA/Matlab};
  ...
  \node (pic1) at (7.40,-3.5) { \includegraphics[width=0.6\linewidth]{H5PartROOT}};
...
\section{Modeling of Neighboring Bunch Effects}
\frame{
...
\frame{
  ...
  \begin{block}{Energy bins}
    ...
    \item Compute each bins contribution separately
...
% - 870 KeV
% - Collimation INJ2 & Round Beam
% - Ring
% AAA
% \rput[lb](2.5cm,-6.5cm){\includegraphics[width=0.8\linewidth,angle=-90]{beamline_without_accelerator.eps}}
...
\frame{
 \frametitle{PSI Injector II}
  ...
  \includegraphics[angle=90,width=0.28\linewidth]{1mA0}
  \includegraphics[angle=90,width=0.28\linewidth]{1mA5}
  \includegraphics[angle=90,width=0.28\linewidth]{1mA10}\\
  \includegraphics[angle=90,width=0.28\linewidth]{1mA20}
  \includegraphics[angle=90,width=0.28\linewidth]{1mA30}
  \includegraphics[angle=90,width=0.28\linewidth]{1mA40}\\
...
\frame{
 ...
 \framesubtitle{\alert{Single bunch} 3 mA: Compare rms-sizes}


%  \begin{block}{Start-to-end RMS sizes comparison of a 3MeV beam}  
    \includegraphics[width=0.5\linewidth]{Comp-Transverse.pdf}
    \includegraphics[width=0.5\linewidth]{Comp-Longitudinal.pdf}
 % \end{block}
}
...
% \begin{itemize}
%      \item 9 bunches for a stable solution
%      \item it has visible impacts on the beam dynamics
%      \item the resulting bunches are more compact in the transverse direction
%      \end{itemize}
%  \end{block}
\end{frame}
...
\frame{
  \frametitle{SwissFEL Injector Test Facility (RIP)}
  \vspace{-3mm}
  \begin{center}
 \includegraphics[angle=0,width=0.8\linewidth]{SwissFELInjectorT} \\
 \includegraphics[angle=0,width=0.6\linewidth]{OPALMeasvsModel} {\small N. P. Neatu}
\end{center}
}
