...
\opalcycl is a fully three-dimensional simulation program dedicated to future high intensity cyclotron 
...
The generic interface function both for 2D measured map and 3D field from file will be available in later version.\\ 
...
In Space charge calculation for multi-bunches, the computation region covers all bunches.
Because the energy of bunches are quite different, it is inappropriate to use only one particle rest frame and single Lorentz transformation any more.
...
space charge field and apply back Lorentz transforming for each bin separately. Then add the field data together. each particle has a ID number to identify 
which energy bin it belongs to.

The programming work on this aspects has not finished yet.

\subsection{Input}  
All three working modes of \opalcycl use an input file written in MAD9 language which will be described in detail in following chapters.

For the  {\bfseries Tune Calculation mode}, one additional auxiliary file with following format is needed.
\begin{verbatim}
   72.000 2131.4   -0.240  
   74.000 2155.1   -0.296  
   76.000 2179.7   -0.319 
   ...
   84.000 2278.0    0.025 
\end{verbatim}
In each line the three values represent energy, reference $P_\theta$ and reference $P_r$ for SEO(Static Equilibrium Orbit) 
at start point respectively and their units are $MeV,  mrad,  mrad$.

An bash script $tuning.sh$ is needed to execute \opalcycl once for each energy. 
\begin{verbatim}
  #!/bin/bash
  rm -rf tempfile
  rm -f plotdata
...
To start execution, just run $tuning.sh$ which uses input file $testcycl.in$ and auxiliary file $FIXPO\_SEO$.
...
using the H5PartRoot (\url{http://amas.web.psi.ch/tools/H5PartROOT/index.html}). The frequency 
of the data output can be set using the \secref{\texttt{OPTION} statement}{option}, statement and the flag PSDUMPFREQ. 
