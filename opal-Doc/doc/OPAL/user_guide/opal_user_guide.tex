...
\newcommand{\opalversion}{\text{1.1.8}}
...
\newcommand{\opalenv}{\textsc{OPAL-e} }
...
\newcommand{\latermore}{More details will be given in Version 1.1.9}
...
\newcommand{\bibref}[2]{#1 \cite{bib:#2}}%\hypercite{#1}{}{}{bib:#2}}

\newcommand{\tabline}[3]{\hyperref{\texttt{#1}}%
  {\texttt{#1} (}{)}{sec:#3} & #2 \\}
\newcommand{\figref}[2]{\hyperref{#1}{#1 (see~Figure~}{)}{fig:#2}}
\newcommand{\chpref}[2]{\hyperref{#1}{#1 (see~Chapter~}{)}{chp:#2}}
...
<br>
Andreas Adelmann, Christof Kraus, Yves Ineichen (PSI), Steve Russell (LANL),\\ Yuanjie Bi, Jianjun Yang (CIAE), and Colwyn Gulliford (Cornell)
</center>
\end{rawhtml}
\end{htmlonly}
...
\vskip 1.5em
\begin{center}                       % author
Andreas Adelmann, Christof Kraus, Yves Ineichen (PSI), Steve Russell (LANL),\\ Yuanjie Bi and Jianjun Yang (CIAE)
\vskip 2em
{\large Abstract}
\end{center}
\end{latexonly}
...
\include{conventions}     	% ch 3 
\include{opal-flavours}    	% ch 4
\include{format}                 	% ch 5
\include{control}              	% ch 6
\include{elements}          	% ch 7
\include{lines}                  	% ch 8
\include{physics}             	% ch 9
\include{tables}               	% ch 10
\include{actions}             	% ch 11
\include{match}              	% ch 12
\include{errors}              	% ch 13
\include{distribution}     	% ch 14
\include{fieldsolvers}    	% ch 15
\include{wakefields}     	% ch 16
\include{geometry}       	% ch 17
\include{track}               	% ch 18
\include{fieldemission}      % ch 19
\include{multipact}              % ch 20 
\appendix
\include{syntax}
\include{fieldmaps}
