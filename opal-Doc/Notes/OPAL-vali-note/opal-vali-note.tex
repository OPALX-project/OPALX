...
\feltitle{
  ...
  IMPACT-T, Astra \& \opal-t Code Comparison
}{
  A. Adelmann, Y. Ineichen, Ch. Kraus,  \\ A. Oppelt (DESY) and S. Russel (LANL)
}{
  \PSI
}{
  \felabstract{
    In this note we compare IMPACT-T, Astra and \opal-t in the SwissFEL context.
  }
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.49\linewidth,angle=0]{figures/opal-astra-xzrms-1}
\includegraphics[width=.49\linewidth,angle=0]{figures/opal-astra-xzrms-2}
...
$n$ processors v.s. the time on one processor multiplied by $1/n$.
\begin{equation}
...
$\sim \bf 6$ on the problem described in section \ref{sec:OPALAstra} (from 5
hours 22 minutes down to 50 minutes) when running \opal~ on the PSI cluster
(Felsim) employing 16 cores.
