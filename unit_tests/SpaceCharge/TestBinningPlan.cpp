...
namespace opalx::spacecharge {
    namespace {
        ...
        class BinningPlanTest : public ::testing::Test {
        protected:
            ...
            [[nodiscard]] BinningConfig config(bool adaptive) const {
                ...
                values.alpha         = 1.0;
                values.beta          = 1.5;
                values.parameter     = BinningParameterKind::VelocityZ;
        ...
        TEST_F(BinningPlanTest, RequiresPreparationBeforeLazyTraversal) {
            BinningPlan<double, 3> plan(*particles_m, config(false));
            EXPECT_THROW(static_cast<void>(plan.next()), OpalException);
        }

        TEST_F(BinningPlanTest, TraversesOnlyNonemptyBinsAndReturnsOptionalSnapshot) {
            BinningPlan<double, 3> plan(*particles_m, config(false));
            const BinningPreparation prepared = plan.prepare(true);
            ...
            while (const auto unit = plan.next()) {
                ++visited;
        ...
        TEST_F(BinningPlanTest, CapturesPostMergeSnapshotOnlyForAdaptiveBinning) {
            BinningPlan<double, 3> plan(*particles_m, config(true));
            const BinningPreparation prepared = plan.prepare(true);
            EXPECT_TRUE(prepared.beforeMerge.has_value());
