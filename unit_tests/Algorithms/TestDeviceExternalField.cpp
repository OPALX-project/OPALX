...
TEST_F(DeviceExternalFieldTest, OrderedCandidatesPreserveOccurrencesAndRejectUnknowns) {
    Beam beam;
    PartBunch_t bunch({1.}, {1.}, {&beam}, {0}, 1., "LF2", opalx::spacecharge::CartesianDomainConfig3D{});
...
TEST_F(DeviceExternalFieldTest, PreparedOrderMakesDeviceFrameAndFieldWorkIndependentOfAddresses) {
    ...
    for (unsigned arrangement = 0; arrangement < 2; ++arrangement) {
        ...
        std::set<std::shared_ptr<ElementBase>> selected;
        for (auto it = allocated.rbegin(); it != allocated.rend(); ++it) selected.insert(*it);
        const auto ordered = device_external::orderedCandidates(lattice, selected);
        ASSERT_EQ(ordered.size(), names.size());
        for (unsigned i = 0; i < names.size(); ++i) EXPECT_EQ(ordered[i]->getName(), names[i]);
        ...
        const auto outR = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), particles->R.getView());
        const auto outP = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), particles->P.getView());
        const auto outE = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), particles->E.getView());
        const auto outB = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), particles->B.getView());
        for (unsigned i = 0; i < 3; ++i) {
...
TEST_F(DeviceExternalFieldTest, ThinMagnetDeviceImpulseEnergyAndSpectators) {
    ...
    rh(0) = rh(2) = Vector(0); rh(1) = Vector(0.1, 0, 0);
...
TEST_F(DeviceExternalFieldTest, DeviceFieldsMatchPlacedHostBendsWithFringesAndMultipole) {
    ...
    for (int kind = 0; kind < 3; ++kind) {
        ...
        sector.setFullGap(0.03); rectangular.setFullGap(0.03);
        sector.setFringeIntegral(0.4); rectangular.setFringeIntegral(0.4);
        quad.setNormalComponent(0, 0.8); quad.setNormalComponent(1, 0.3);
        quad.setSkewComponent(0, 0.1); quad.setSkewComponent(1, -0.2);
        ...
        const CoordinateSystemTrafo frame(Vector(1., -2., 0.3), Quaternion(std::cos(0.2), 0., std::sin(0.2), 0.));
        element.setCSTrafoGlobal2Local(frame); element.fixPosition();
        ...
        const Vector local[] = {Vector(0.01, 0.02, -0.02), Vector(0.01, 0.02, 0.02),
            ...
            Vector(0, 0, 1.04), Vector(0.5, 0, 0.5), Vector(0, 0.2, 0.5)};
        for (unsigned i = 0; i < 7; ++i) input(i) = frame.transformFrom(local[i]);
        Kokkos::deep_copy(points, input);
        Kokkos::parallel_for("test spatial field", 7, KOKKOS_LAMBDA(int i) {
        ...
        for (unsigned i = 0; i < 7; ++i) {
            ...
            for (unsigned d = 0; d < 3; ++d) EXPECT_NEAR(output(i)(d), expected(d), 2e-13);
        }
    }
}
...
TEST_F(DeviceExternalFieldTest, FrameTransformAndReverseDriftStayOnDevice) {
    ...
    const CoordinateSystemTrafo frame(Vector(1, 2, 3), Quaternion(std::cos(0.3), 0., std::sin(0.3), 0.));
    ...
    Kokkos::deep_copy(r, start); Kokkos::deep_copy(p, momentum);
    ...
    for (unsigned d = 0; d < 3; ++d)
        EXPECT_NEAR(forward(0)(d), start(d) + Physics::c * 1e-10 * momentum(d) / std::sqrt(1 + dot(momentum, momentum)), 2e-15);
    ASSERT_EQ(lattice.transport(r, p, e, b, 1, deviceFrame, -1e-10, 1e9, -1.), 0);
    const auto backward = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), r);
    for (unsigned d = 0; d < 3; ++d) EXPECT_NEAR(backward(0)(d), start(d), 2e-15);
...
TEST_F(DeviceExternalFieldTest, UniformRFCavityGeometryUsesItsWidthAndRejectsMagneticOnlyOracle) {
    ...
    Kokkos::parallel_for("RF support check", 8, KOKKOS_LAMBDA(int i) {
        flags(i) = device.elements(0).contains(points(i));
    });
    const auto actual = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), flags);
    for (int i = 0; i < 8; ++i) EXPECT_EQ(bool(actual(i)), cavity.isInside(input(i)));
...
TEST_F(DeviceExternalFieldTest, RejectsUnimplementedDeviceFieldInsteadOfHostFallback) {
    ...
    sextupole.setCSTrafoGlobal2Local(CoordinateSystemTrafo()); sextupole.fixPosition();
    ...
    EXPECT_FALSE(device_external::Builder::supports(lattice));
    EXPECT_THROW(device_external::Builder::build(lattice), OpalException);
    stored->setMisalignment(CoordinateSystemTrafo(Vector(0), Quaternion(std::cos(0.1), 0., std::sin(0.1), 0.)));
    EXPECT_FALSE(device_external::Builder::supports(lattice));
    EXPECT_THROW(device_external::Builder::build(lattice), OpalException);
...
TEST_F(DeviceExternalFieldTest, EligibilityAcceptsPassivesAndRejectsOtherFieldModelsWithoutMutation) {
    Beam beam;
    PartBunch_t bunch({1.}, {1.}, {&beam}, {0}, 1., "LF2", opalx::spacecharge::CartesianDomainConfig3D{});
    FlaggedBeamline line;
    DefaultVisitor visitor(line, false, false);
...
TEST_F(DeviceExternalFieldTest, CyclotronSectorRetainsItsMappedTrackingPath) {
            ...
            / ("opalx-ring-eligibility-" + std::to_string(getpid()) + ".map");
    {
        ...
        for (int i = 0; i < 13; ++i) out << "header ";
        out << "5 8 a b c d e 2 a b c d 0.100000000E+01-0.100000000E+01 a b c d e f LREC= a b c d e\n";
        for (int i = 0; i < 5; ++i) {
            if (i) out << "a b c d e f\n";
            for (int channel = 0; channel < 4; ++channel)
                for (int j = 0; j < 8; ++j) out << (channel == 0 ? 10. : 0.) << ' ';
            out << '\n';
        }
    }
...
TEST_F(DeviceExternalFieldTest, EligibilityRejectsHigherBendComponentsButAllowsZeroPadding) {
    ...
    for (unsigned kind = 0; kind < 2; ++kind) {
        ...
        for (bool normal : {true, false}) {
            ...
            if (kind == 0)
                dynamic_cast<SBend&>(*stored).setFieldComponents(normalCoefficients, skewCoefficients);
            else
                dynamic_cast<RBend&>(*stored).setFieldComponents(normalCoefficients, skewCoefficients);
            EXPECT_FALSE(device_external::Builder::supports(lattice));
            EXPECT_THROW(device_external::Builder::build(lattice), OpalException);
        }
    }
