...
TEST_F(UniformTest, SamplesColdUniformEllipsoid) {
    ...
    Kokkos::parallel_reduce(
            pc->getLocalNum(),
            KOKKOS_LAMBDA(
                    const size_t i, Vector_t<double, 3>& localSum, Vector_t<double, 3>& localSum2,
                    double& maxRadius, double& maxPError) {
                localSum += R(i);
                localSum2 += R(i) * R(i);
                ...
                maxPError = Kokkos::max(
                        maxPError,
                        Kokkos::abs(P(i)[0]) + Kokkos::abs(P(i)[1]) + Kokkos::abs(P(i)[2] - pz));
            },
