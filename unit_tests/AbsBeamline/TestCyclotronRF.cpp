...
protected:
    ...
    void SetUp() override {
        filename = (std::filesystem::temp_directory_path()
                    / ("opalx-rf-" + std::to_string(getpid()) + ".dat"))
                           .string();
...
TEST_F(CyclotronRFTest, EnergyGainTransitTimeAndPhase) {
    ...
    for (double width : {0., .3}) {
        for (double phase : {0., Physics::pi, Physics::pi / 2}) {
            ...
            EXPECT_NEAR(
                    (std::sqrt(1 + dot(p, p)) - gamma) * mass, 847000 * transit * std::cos(phase),
                    3e-7);
        }
    }
}
...
TEST_F(CyclotronRFTest, FocusingDeviceParityAndInvalidState) {
    ...
    const double g1 =
            gamma + 847000 * (1 + std::pow(.6, 3)) * std::sin(a) / a * std::cos(phi) / mass;
    ...
    Kokkos::parallel_for("rf kick test", 1, CyclotronRFKickTestFunctor{grid, output, kick});
    const auto host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), output);
    EXPECT_EQ(host(3), 1);
    for (int i = 0; i < 3; ++i)
        EXPECT_NEAR(host(i), expected[i], 1e-14);
...
TEST_F(CyclotronRFTest, GapCloneAndNoContinuousFieldContribution) {
    ...
    cavity.configureCyclotronGap(
            filename, 1, 2, CyclotronRFKick{847000, Physics::two_pi * 50.65e6, 0, .3, 1});
