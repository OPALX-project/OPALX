...
TEST_F(OpalBeamlinePlacementTest, CurvedMultipoleTIsFramedLikeAnSBend) {
    ...
    // MULTIPOLET turns the reference path exactly as an SBEND of the same arc and angle
    // does: its own entrance frame and the frame of the following element agree.
    ...
    magnet->getGeometry() = Geometry::makeSBend(arc, angle / arc);
    magnet->setTransverseProfile({-1.0});
    magnet->setFringeLengths(0.01, 0.01);
