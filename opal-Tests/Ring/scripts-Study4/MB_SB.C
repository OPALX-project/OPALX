...
void MB_SB(int Nstep=0, int NB=1){
  ...
  // load the data file:
  TH5Dataset *data1 = new TH5Dataset("Scenario-3/Ring-4.h5");
  TH5Dataset *data2 = new TH5Dataset("Scenario-6/Ring-4.h5");
  int Nstep_MB=Nstep+(NB-1)/2;
  int Nstep_SB=Nstep;

  cout<<"loaded step of single bunch file = "<<Nstep_SB<<endl;
  cout<<"loaded step of multi-bunch  file = "<<Nstep_MB<<endl;
  cout<<"Bunches Num. of multi-bunch file = "<<NB<<endl;
 ...
 // for convenience, get a dummy canvas for plotting
  ...
  cx = tcx->GetY();
  cy = tcy->GetY();
  angle = calculateAngle( cx[Nstep_MB], cy[Nstep_MB]) ;
  cosAngle[1];
  sinAngle[1];
  cosAngle[0] = cos(angle);
  sinAngle[0] = sin(angle);
  ...
  TNtupleD *ntd = data2->GetNtuple(Nstep_MB);
  TH1D* h1dX2 = new TH1D("h1dX2","h1 title",256,XminL,XmaxL);
  ntd->Draw(Form("1000.0*((x-%g)*%g+(y-%g)*%g)>>h1dX2",cx[Nstep_MB],cosAngle[0],cy[Nstep_MB],sinAngle[0]));
  ...
  TLegend *legend1=new TLegend(0.67,0.57,0.95,0.80);    
  ...
  h1dX1->SetYTitle("Number of particles");
  h1dX1->SetFillColor(38);
  h1dX1->SetAxisRange(0, 2500, "Y");
  ...
  legend1->AddEntry(h1dX2,"9 bunches","l");
  ...
  TString s1 = TString("test-")+Nstep+".pdf";
  TString s2 = TString("test-")+Nstep+".png";
