...
void C9B_7B_SB_Energy_1mA(int Nstep=0, int NB1=1, int NB2=1){
 ...
 // for convenience, get a dummy canvas for plotting
  ...
  /////////////////
  // Double_t gamma = data1->GetScalarStepVar("GammaBin", Nstep_SB);
  ...
  */
  Double_t E0 = 938.3;
  // this range is especially for Nstep_SB = 80
  Double_t min1 = (1.31-1)*938.3;
  Double_t max1 = (1.315-1)*938.3;
  // this range is especially for Nstep_SB = 130
  Double_t min1 = (1.482-1)*938.3;
  Double_t max1 = (1.491-1)*938.3;
  Int_t bins=200;
  TCanvas* dummy = new TCanvas("dummy","dummy",10, 10);      
  //TH1D* h1dX1 = data1->Histo1d("sqrt(1+px*px+py*py+pz*pz)", Nstep_SB, min1, max1, bins,"sqrt(1+px*px+py*py+pz*pz)>1.482&&sqrt(1+px*px+py*py+pz*pz)<1.49");
  //TH1D* h1dX2 = data2->Histo1d("sqrt(1+px*px+py*py+pz*pz)", Nstep_MB1,min1, max1, bins,"sqrt(1+px*px+py*py+pz*pz)>1.482&&sqrt(1+px*px+py*py+pz*pz)<1.49");
  //TH1D* h1dX3 = data3->Histo1d("sqrt(1+px*px+py*py+pz*pz)", Nstep_MB2, min1, max1, bins,"sqrt(1+px*px+py*py+pz*pz)>1.482&&sqrt(1+px*px+py*py+pz*pz)<1.49");

  TH1D* h1dX1 = data1->Histo1d("938.3*(sqrt(1+px*px+py*py+pz*pz)-1)", Nstep_SB, min1, max1, bins,"");
  TH1D* h1dX2 = data2->Histo1d("938.3*(sqrt(1+px*px+py*py+pz*pz)-1)", Nstep_MB1,min1, max1, bins,"");
  TH1D* h1dX3 = data3->Histo1d("938.3*(sqrt(1+px*px+py*py+pz*pz)-1)", Nstep_MB2, min1, max1, bins,"");
  delete dummy;
  //--------------plot--------------------//
  ...
  h1dX1->SetXTitle("Kinetic energy (MeV) ");
  h1dX1->SetYTitle("Density (a.u.)");
  h1dX1->SetFillColor(38);
  h1dX1->SetAxisRange(0, 4400, "Y");
  ...
  TString s1 = TString("C9B7BSB-Energy-1mA-")+Nstep+".pdf";
  //  TString s2 = TString("C9B7BSB-Energy-1mA-")+Nstep+".png";
