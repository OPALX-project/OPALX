...
# execute OPAL to calculate tuning frquency and store
  opal testcycl-3.in --commlib mpi --info 0 | grep "Max" >>tuningresult
   j=$[$j+1]
done
exec 6<&-
rm -rf data.dat
...
# post porcess
# read in the result of FIXPO and OPAL together into file plotdata  
exec 8<tuningresult
exec 9<FIXPOUdata 
rm -f plotdata 
i="0"
...
# format : eneryg, ur_fixpo, uz_fixpo, ur_opal-cycl, ur_opal-cycl
  echo "$E   $ur2   $uz2   $ur1   $uz1" >>plotdata 
