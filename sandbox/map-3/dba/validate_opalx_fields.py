...
def main():
    ...
    parser.add_argument("--require-current-build", action="store_true",
                        help="Additionally require the original executable hash on this machine")
    args = parser.parse_args()
    p = load_parameters()
    provenance = json.loads((args.output / "provenance.json").read_text())
    for name, sha in provenance["source_hashes"].items():
        path = REPO / name
        if name == "sandbox/map-3/dba/run_best_cases.py" and digest(path) != sha:
            path = args.output / "source/run_best_cases.py"
