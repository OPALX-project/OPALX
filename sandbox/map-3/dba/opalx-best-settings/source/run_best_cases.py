...
def main():
    ...
    provenance = dict(created_utc=datetime.now(timezone.utc).isoformat(), ranks=1, omp_threads=1,
                      sequential=True, selection_rule="minimum full-map error among all saved valid map-2 single-rank cases",
                      selection_table=str(SELECTION), selection_table_sha256=digest(SELECTION),
